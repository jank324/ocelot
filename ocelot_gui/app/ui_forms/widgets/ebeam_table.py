...
class EbeamWindow(QtWidgets.QWidget):
    ...
    def calc_ebparams(self):
        ...
        ebparams = EbeamParams(
            self.lattice.lattice,
            self.tws.tws[0],
            nsuperperiod=self.lattice.nsuperperiods,
        )

        Q = [
            self.tws.tws[-1].mux / 2.0 / np.pi * self.lattice.nsuperperiods,
            self.tws.tws[-1].muy / 2.0 / np.pi * self.lattice.nsuperperiods,
        ]
        q_ksi = natural_chromaticity(
            self.lattice.lattice, self.tws.tws[0], self.lattice.nsuperperiods
        )
        ...
        ebp.append(
            self.format_ebp(
                [self.lattice.nsuperperiods * self.lattice.lattice.totalLen], 3
            )
        )
        ...
        ebp.append(
            self.format_ebp([ebparams.tau_x, ebparams.tau_y, ebparams.tau_e], 11)
        )
        ebp.append(
            self.format_ebp(
                [ebparams.I1, ebparams.I2, ebparams.I3, ebparams.I4, ebparams.I5], 11
            )
        )
