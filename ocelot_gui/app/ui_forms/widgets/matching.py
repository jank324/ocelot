...
class MatchingWindow(QtWidgets.QWidget):
    ...
    def prepare_columbs(self):
        for label in self.ui.tws_labels:
            ...
            if val != 0.0:
                ...
                self.ui.__dict__["lineEdit_" + label + "_in"].setText(
                    str(round(val, 6))
                )
    ...
    def action_matching(self):
        ...
        for line in self.matching_vars:
            ...
            new_lines += (
                ...
                + str(line["type"])
                + " = "
                + str(line["element"].__dict__[line["type"]])
                + "\n"
            )
            ...
            for type, panel in self.tune_elements[elem_id].items():
                if panel.is_selected():
                    if type == self.lattice.matchable_elements[elem_class]:
                        self.matching_vars.append(
                            {
                                "element": element,
                                "elem_id": elem_id,
                                "type": type,
                                "init_val": element.__dict__[type],
                            }
                        )
                    else:
                        string += (
    ...
    def action_reset_results(self):
        ...
        for line in self.matching_vars:
            ...
            self.tune_elements[line["elem_id"]][line["type"]].set_curval(
                line["init_val"], block_signal=True
            )
