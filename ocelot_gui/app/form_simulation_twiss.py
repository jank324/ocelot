...
class TwissMonitor(QtWidgets.QWidget):
    ...
    def init_twiss(self):
        ...
        self.twiss_plot.plot_lattice.setXRange(
            0.0, self.lattice.lattice.totalLen, padding=0
        )
    ...
    def action_open_matching_window(self):
        self.child_windows["matching"] = MatchingWindow(
            self.lattice, self.tune_elements
        )
        ...
        for elem_id, element in self.lattice.elements.items():
            if element.is_tuneable:
                for type in element.tune_params:
                    element.__dict__[type] = self.mw.lattice.elements[elem_id].__dict__[
                        type
                    ]
    ...
    def action_update_lattice(self):
        ...
        for elem_id, element in self.lattice.elements.items():
            if element.is_tuneable:
                for type in element.tune_params:
                    self.mw.lattice.elements[elem_id].__dict__[type] = element.__dict__[
                        type
                    ]
