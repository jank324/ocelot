    ...
    def calc_lattice_elements_pics(self):
        ...
        for elem in self.lattice.lattice.sequence:
            ...
            if elem.__class__ == Edge:
                ...
                elements["cavs"].append(
                    [2.0, pos - d_pos, -1.0 - self.h_drift, l, elem]
                )
                ...
                elements["drifts"].append(
                    [2.0 * self.h_drift, pos - d_pos, -self.h_drift, l, elem]
                )
        ...
        # add white line
        item = callbackElement(
            0.0,
            -self.h_drift * self.scale,
            self.lattice.lattice.totalLen,
            2.0 * self.h_drift * self.scale,
        )
        ...
        for elem in elements["drifts"]:
            item = callbackElement(
                elem[1], elem[2] * self.scale, elem[3], elem[0] * self.scale
            )
            item.element = elem[4]
            item.setAcceptHoverEvents(True)
