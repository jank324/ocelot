...
def slit_func(xc, yc):
    for i in range(len(xc)):
        for j in range(len(yc)):
            is_slit = False
            if np.abs(xc[i] - 0.05) < 0.01 and np.abs(yc[j] - 0.00) < 0.01:
                is_slit = True
            if np.abs(xc[i] + 0.05) < 0.01 and np.abs(yc[j] - 0.00) < 0.01:
                is_slit = True
            if np.abs(xc[i] + 0.00) < 0.01 and np.abs(yc[j] - 0.05) < 0.01:
                is_slit = True
            if np.abs(xc[i] + 0.00) < 0.01 and np.abs(yc[j] + 0.05) < 0.01:
                is_slit = True
            if is_slit:
...
xc = np.linspace(-5.0 * sigx, 5.0 * sigx, 20)
xpc = np.linspace(-5.0 * sigxp, 5.0 * sigxp, 20)


# yo = np.linspace(-0.2,0.2,50)
yo = np.linspace(-1.0e-1, 1.0e-1, 50)
...
xc = np.linspace(-5.0 * sigx, 5.0 * sigx, 250)
xpc = np.linspace(-5.0 * sigxp, 5.0 * sigxp, 250)
