...
screen_no = calculate_radiation(
    lat, deepcopy(screen), beam, energy_loss=False, quantum_diff=False
)
...
for i in range(npls):
    ...
    screen = calculate_radiation(
        lat, deepcopy(screen), beam, energy_loss=False, quantum_diff=True
    )
    total += screen.Total / npls
    Uq.append(screen.Ef_electron)

sigma = sigma_gamma_quat(
    beam.E, U40_short.Kx, U40_short.lperiod, U40_short.lperiod * nund * nperiods
)
...
n, bins, patches = plt.hist(
    (np.array(Uq) - beam.E) / beam.E, num_bins, normed=1, facecolor="green", alpha=0.5
)
