...
method = LinacRmatrixRM(
    lattice=orb.lat, hcors=orb.hcors, vcors=orb.vcors, bpms=orb.bpms
)
...
fig, ax = plot_API(lat)
ax.plot(s_bpm_b, x_bpm_b * 1000.0, "ro-", label="X [mm]")
ax.plot(s_bpm_b, y_bpm_b * 1000.0, "bo-", label="Y [mm]")
ax.legend()
plt.show()
...
print(
    f"{orb.orbit_solver.__class__.__name__} method: ||HVcor|| = {np.sqrt(np.sum(hcors_angle * hcors_angle) + np.sum(vcors_angle * vcors_angle))}"
)
print(
    f"{orb.orbit_solver.__class__.__name__} method: ||XY_bpm|| = {np.sqrt(np.sum(x_bpm * x_bpm) + np.sum(y_bpm * y_bpm))}"
)
...
fig, ax = plot_API(lat)
ax.plot(s_bpm_b, x_bpm * 1000.0, "ro", label="X [mm]")
ax.plot(s, x, "r")
ax.plot(s_bpm_b, y_bpm * 1000.0, "bo", label="Y [mm]")
ax.plot(s, y, "b")
plt.show()

rm = orb.response_matrix.extract(
    cor_list=["CIX.78.I1", "CIY.80.I1"], bpm_list=["BPMA.85.I1", "BPMA.87.I1"]
)
print(rm)
...
rm[1, 1] = -0.1
print(rm)
orb.response_matrix.inject(
    cor_list=["CIX.78.I1", "CIY.80.I1"],
    bpm_list=["BPMA.85.I1", "BPMA.87.I1"],
    inj_matrix=rm,
)
rm_check = orb.response_matrix.extract(
    cor_list=["CIX.78.I1", "CIY.80.I1"], bpm_list=["BPMA.85.I1", "BPMA.87.I1"]
)
