...
from ocelot.adaptors.genesis import beam2edist, edist2beam
...
dg0 = 6
beam.beta_x = beta_x0 * (
    1 + 0.008 * (beam.s - s0) ** 2 / 2 / (speed_of_light * 1e-15) ** 2
)
beam.beta_y = beta_x0 * (
    1 - 0.4 + 0.006 * (beam.s - s0) ** 2 / 2 / (speed_of_light * 1e-15) ** 2
)
beam.emit_x = emit_x0 * (
    1 + 0.2 + 0.05 * (beam.s - s0) ** 2 / 2 / (speed_of_light * 1e-15) ** 2
)
beam.emit_y = emit_y0 * (
    1 + 0.01 * (beam.s - s0) ** 2 / 2 / (speed_of_light * 1e-15) ** 2
)
...
beam.add_chirp_poly(coeff=[0.0, +5.0, -1, -0, 0.01], s0=None)
