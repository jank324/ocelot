...
e_beam_param = (
    "N_x = {}, ".format(round((ebeam_sigma_x) ** 2 / xlamds / L_w, 3))
    + "N_y = {}, ".format(round((ebeam_sigma_y) ** 2 / xlamds / L_w, 3))
    + "D_x = {}, ".format(round((ebeam_sigma_xp) ** 2 * L_w / xlamds, 3))
    + "D_y = {}, ".format(round((ebeam_sigma_yp) ** 2 * L_w / xlamds, 3))
    + "N_b = {} ".format(N_b)
)
print(e_beam_param)
...
dfl_SERVAL.fld = np.random.randn(
    dfl_SERVAL.Nz(), dfl_SERVAL.Ny(), dfl_SERVAL.Nx()
) + 1j * np.random.randn(
    dfl_SERVAL.Nz(), dfl_SERVAL.Ny(), dfl_SERVAL.Nx()
)  # Gaussian noise
...
dfl_SERVAL = dfl_gen_undulator_serval(
    dfl_SERVAL,
    L_w=L_w,
    sig_x=ebeam_sigma_x,
    sig_y=ebeam_sigma_y,
    sig_xp=ebeam_sigma_xp,
    sig_yp=ebeam_sigma_yp,
    k_support="intensity",
    s_support="intensity",
    showfig=False,
)
