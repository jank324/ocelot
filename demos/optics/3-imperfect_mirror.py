...
from ocelot.gui.dfl_plot import plot_1d_hprofile, plot_dfl
...
hprofile = generate_1d_profile(
    ...
    seed=666,
)  # seed for np.random.seed() to allow reproducibility
...
dfl_reflect_surface(
    dfl,  # ocelot.optics.wave.RadiationField, which will be reflected from imperfect mirror surface (hprofile2)
    ...
    axis="x",
)  # direction along which reflection takes place
...
plot_dfl(
    dfl, phase=1, fig_name="radiation after reflection from mirror1 and propagation"
)
...
dfl_reflect_surface(
    dfl2,  # ocelot.optics.wave.RadiationField, which will be reflected from imperfect mirror surface (hprofile2)
...
dfl2.prop(z=10)
plot_dfl(
    dfl2, phase=1, fig_name="radiation after reflection from mirror2 and propagation"
)
...
hprofile3 = dfl_reflect_surface(
    dfl3,  # ocelot.optics.wave.RadiationField, which will be reflected from imperfect mirror surface (hprofile2)
    angle=np.pi * 2 / 180,  # [radians] angle of incidence with respect to the surface
    hrms=1e-9,  # [m] height errors root mean square
    axis="x",  # direction along which reflection takes place
    return_height_profile=True,
)  # boolean type variable; if it equals True the function will return internally generated height_profile

plot_1d_hprofile(
    hprofile3, fig_name="internally generated mirror3 height profile and PSD"
)
dfl3.prop(z=10)
plot_dfl(
    dfl3,
    fig_name="radiation after reflection internally generated mirror3 and propagation",
)
