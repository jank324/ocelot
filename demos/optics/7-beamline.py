...
kwargs = {
    "xlamds": (h_eV_s * speed_of_light / E_pohoton),  # [m] - central wavelength
    "rho": 1.0e-4,
    ...
    "power_rms": (
        ...
        50e-6,
        3e-6,
    ),  # (x,y,z) [m] - rms size of the radiation distribution (gaussian)
    "power_center": (
        0,
        0,
        None,
    ),  # (x,y,z) [m] - position of the radiation distribution
    "power_angle": (0, 0),  # (x,y) [rad] - angle of further radiation propagation
    "power_waistpos": (
        0,
        0,
    ),  # (Z_x,Z_y) [m] downstrean location of the waist of the beam
...
MirrorSurface = ImperfectMirrorSurface(
    hrms=3e-9, angle=10 * np.pi / 180, plane="x", eid="MirrorSurface"
)
...
FreeSpace_to_the_sample = FreeSpace(
    l=l_b, mx=0.1, my=0.1, eid="free_space_to_the_sample"
)
# %%
...
# %%
line = (
    FreeSpace_before_lens,
    Aperture,  # set of optical elements
    FreeSpace_after_aperture,
    MirrorSurface,
    Lens,
    FreeSpace_to_the_sample,
)

dfl_prop = deepcopy(dfl)
lat = OpticsLine(
    line, start=FreeSpace_before_lens, stop=FreeSpace_before_lens
)  # defining a beamline lattice  with "start" and "stop" elements for propagation
dfl_prop = propagate(lat, dfl_prop)
plot_dfl(
    dfl_prop, fig_name="{}".format(FreeSpace_before_lens.eid), phase=True, domains="sf"
)
...
plot_dfl(
    dfl_aperture,
    domains="s",
    fig_name="{}".format(FreeSpace_after_aperture.eid),
    phase=True,
)
plot_dfl(
    dfl_aperture,
