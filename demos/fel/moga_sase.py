...
beam_fileName = (
    param_dir + "beams/beam_0.02nC_aftSASE1_gianluca.txt"
)  # path to beam file
...
beam = transform_beam_twiss(
    beam,
    transform=[[beam_pk.beta_x, beam_pk.alpha_x], [beam_pk.beta_y, beam_pk.alpha_y]],
)
...
init_pop.append(
    (
        ...
        np.random.uniform(und.Kx * 0.999, und.Kx * 1.001),
        np.random.uniform(-0.005, 0.0),
...
def fit_func(x0, iter_data, args):
    ...
    inp.isrsig = (
        1  # use it wisely, due to causality problems (affects initial parameters?)
    )
    inp.ncar = ncar
    inp.zstop = zstop
    inp.dgrid = dgrid
...
opt.set_params(
    n_gen=10, log_file=exp_dir + "moga_result.dat", plt_file=exp_dir + "moga_plot.dat"
)
