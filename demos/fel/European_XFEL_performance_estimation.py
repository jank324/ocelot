...
plt.pcolormesh(
    x_arr, y_arr, (z_sat_magn_arr / und_duty_factor) * K_avail, cmap=cmap, vmin=0
)  # , vmax=L_magn / und_duty_factor * 2)
plt.colorbar()
...
if np.any(z_sat_magn_arr / L_magn < 1) and np.any(z_sat_magn_arr / L_magn > 1):
    ...
    plt.clabel(
        ...
        inline=1,
        fontsize=10,
        fmt=" undulator {} m".format(L_magn / und_duty_factor),
    )
...
plt.pcolormesh(
    x_arr, y_arr, E_arr * 1e6 * K_avail, cmap=cmap
)  # , norm=colors.LogNorm())#vmin=1e8, vmax=1e12
plt.colorbar()
plt.xlim(xlim)
plt.ylim(ylim)
plt.xlabel(x_label)
