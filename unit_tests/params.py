...
def check_matrix(
    ...
    result = []
    for (index, x), (index_ref, x_ref) in zip(
        np.ndenumerate(matrix), np.ndenumerate(matrix_ref)
    ):
        result.append(
            check_value(
                x,
...
def check_dict(
    dict_t, dict_ref, tolerance=1.0e-15, tolerance_type="relative", assert_info=""
):
