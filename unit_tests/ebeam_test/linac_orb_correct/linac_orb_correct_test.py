...
def test_response_matrix(lattice, update_ref_values=False):
    ...
    response_matrix_ref = json2numpy(
        json_read(REF_RES_DIR + sys._getframe().f_code.co_name + ".json")
    )

    result = check_matrix(
        orb.response_matrix.matrix,
        response_matrix_ref,
        TOL,
        assert_info=" response_matrix - ",
    )
    assert check_result(result)
...
def test_save_load_rm(lattice, update_ref_values=False):
    ...
    result = check_matrix(
        orb.response_matrix.get_matrix(),
        response_matrix_ref,
        TOL,
        assert_info=" response_matrix - ",
    )
...
def test_inject_extract_rm(lattice, update_ref_values=False):
    ...
    result = check_matrix(
        rm, response_matrix_ref, TOL, assert_info=" response_matrix - "
    )
    assert check_result(result)
...
def test_correction(lattice, update_ref_values=False):
    ...
    result1 = check_matrix(
        x_bpm_b, bpm_ref[0], TOL, assert_info=" x_bpm_before_correction - "
    )
    result2 = check_matrix(
        y_bpm_b, bpm_ref[1], TOL, assert_info=" y_bpm_before_correction - "
    )
    ...
    result5 = check_matrix(
        hcors_angle, bpm_ref[4], TOL, assert_info=" hor corrector - "
    )
    result6 = check_matrix(
        vcors_angle, bpm_ref[5], TOL, assert_info=" ver corrector - "
    )
