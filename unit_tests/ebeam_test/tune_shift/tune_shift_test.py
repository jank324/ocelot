...
def test_track_nturns(lattice, update_ref_values=False):
    ...
    result = check_dict(
        pxy_list, pxy_list_ref, TOL, "absolute", assert_info=" pxy_list track_nturns - "
    )
    assert check_result(result)
...
def track_nturns_wrapper(lattice):
    ...
    if not hasattr(pytest, "ts_pxy_list"):
        ...
        pytest.ts_pxy_list = track_nturns(
            lattice,
            nturns,
            pytest.ts_pxy_list,
            nsuperperiods=1,
...
def teardown_function(function):
    f = open(pytest.TEST_RESULTS_FILE, "a")
    f.write(
        " execution time is " + "{:.3f}".format(time() - pytest.t_start) + " sec\n\n"
    )
    f.close()
