...
def test_track_without_sp_bounds(
...
def test_track_with_sp(lattice, p_array, parameter=None, update_ref_values=False):
    ...
    result1 = check_dict(
        tws_track, tws_track_p_array_ref["tws_track"], 1e-6, assert_info=" tws_track - "
    )
    result2 = check_dict(
        p,
...
def track_wrapper(lattice, p_array, param, bounds=None):
    ...
    if pytest.sp_track_list[param] is None or pytest.sp_p_array[param] is None:
        ...
        pytest.sp_track_list[param], pytest.sp_p_array[param] = track(
            lattice, p_array, navi, bounds=bounds
        )
