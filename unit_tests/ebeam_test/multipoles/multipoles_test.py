...
def test_natural_chromaticity(lattice, update_ref_values=False):
    ...
    natural_ksi_ref = json2numpy(
        json_read(REF_RES_DIR + sys._getframe().f_code.co_name + ".json")
    )

    result = check_matrix(
        natural_ksi, natural_ksi_ref, TOL, assert_info=" natural chromaticity - "
    )
    assert check_result(result)
...
def test_create_track_list(lattice, update_ref_values=False):
    ...
    result = check_dict(
        track_list, track_list_ref, TOL, "absolute", assert_info=" track_list - "
    )
    assert check_result(result)
...
def test_track_nturns(lattice, update_ref_values=False):
    ...
    track_list = track_nturns(
        lattice, nturns, track_list, save_track=True, print_progress=False
    )
    ...
    track_list_stable_ref = json_read(
        REF_RES_DIR + sys._getframe().f_code.co_name + ".json"
    )

    result = check_dict(
        track_list_stable, track_list_stable_ref, TOL, assert_info=" p_list - "
    )
    assert check_result(result)
...
def create_track_list_wrapper():
    if not hasattr(pytest, "multipoles_track_list"):
        ...
        pytest.multipoles_track_list = create_track_list(
            x_array, [0.0], [0.0], energy=0.0
        )
