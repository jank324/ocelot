...
def test_lattice_transfer_map(lattice, parameter, update_ref_values=False):
    ...
    result = check_matrix(
        matrices[parameter], matrix_ref, TOL, assert_info=" r_matrix - "
    )
...
def test_lattice_transfer_map_at_each_step(lattice, parameter, update_ref_values=False):
    ...
    result = check_matrix(
        matrices[parameter][111], matrix_ref, TOL, assert_info=" r_matrix - "
    )
    assert check_result(result)
...
def test_track_with_energy_shift(lattice, parameter, update_ref_values=False):
    ...
    p_array = generate_parray(
        sigma_x=np.sqrt(emitt * tws0.beta_x),
        sigma_px=np.sqrt(emitt * tws0.gamma_x),
        energy=0.13,
    ...
    # tracking
    pytest.tws_track, pytest.p_array = track(
        lattice, p_array, navi=navi, calc_tws=True, print_progress=False
    )
