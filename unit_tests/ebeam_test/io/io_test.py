...
from ocelot.adaptors.astra2ocelot import exact_xp_2_xxstg_mad, exact_xxstg_2_xp_mad
...
def test_ast_mad_transf(p_array, parameter=None, update_ref_values=False):
    ...
    result2 = check_matrix(
        xxstg.T, P, tolerance=1.0e-10, tolerance_type="relative", assert_info="rp"
    )
    assert check_result(result2)
...
def test_ast(p_array, parameter=None, update_ref_values=False):
    ...
    result2 = check_dict(
        p_rel, p_ref, tolerance=1.0e-7, tolerance_type="absolute", assert_info=" p - "
    )
