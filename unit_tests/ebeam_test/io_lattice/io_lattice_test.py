...
def test_original_lattice_transfer_map(
    lattice, tws0, method, parameter=None, update_ref_values=False
):
...
def test_lattice_save_as_py_file(
    lattice, tws0, method, parameter=None, update_ref_values=False
):
    """R maxtrix calculation test"""
    ...
    for i, elem in enumerate(lattice.sequence):
        if (
            elem.__class__.__name__ == new_lat.sequence[i].__class__.__name__
            and elem.id == new_lat.sequence[i].id
...
def test_lattice_save_as_py_file_w_coupler(
    lattice, tws0, method, parameter=None, update_ref_values=False
):
    """R maxtrix calculation test"""
...
def test_lat2input(lattice, tws0, method, parameter, update_ref_values=False):
    ...
    try:
        ...
        assert check_result(
            [
                "Exception error during the lattice file execution, parameter is "
                + str(parameter)
            ]
        )
    ...
    if parameter:
        if "tws0" in loc_dict:
            ...
            assert check_result(
                ["No tws0 in the lattice file, parameter is " + str(parameter)]
            )
    else:
        tws0_new = tws0
    ...
    if "cell" in loc_dict:
        ...
        assert check_result(
            ["No cell variable in the lattice file, parameter is " + str(parameter)]
        )
...
def lattice_new_transfer_map_check(lattice, tws0):
    ...
    r_matrix_ref = json2numpy(
        json_read(REF_RES_DIR + "test_original_lattice_transfer_map.json")
    )

    result = check_matrix(
        r_matrix, r_matrix_ref, TOL, assert_info=" r_matrix for new lattice - "
    )
    assert check_result(result)


@pytest.mark.parametrize("parameter", [0, 1, 2])
def test_lattice_transfer_maps_check(
    lattice, tws0, method, parameter, update_ref_values=False
):
...
def twiss_new_check(lattice, tws0):
    ...
    result = check_dict(
        tws, tws_ref, TOL, "absotute", assert_info=" tws for new lattice - "
    )
    assert check_result(result)
...
def test_merger(lattice, tws0, method, parameter=None, update_ref_values=False):
    ...
    b = Bend(l=1.2, angle=0.02, k1=-1, k2=0.9, eid="bend")
    ...
    b2 = RBend(l=1.2, angle=0.01, k1=-1, k2=-0.9, eid="bend")
    b3 = SBend(l=1.2, angle=0.02, k1=1, k2=0.5, eid="bend")
    ...
    new_lat = merger(
        lat, remaining_types=[], remaining_elems=[], init_energy=init_energy
    )
...
def test_merger_elem(lattice, tws0, method, parameter=None, update_ref_values=False):
    ...
    R = lattice_transfer_map(lat, energy=init_energy)
    new_lat = merger(
        lat, remaining_types=[], remaining_elems=[sol], init_energy=init_energy
    )
    R_new = lattice_transfer_map(new_lat, energy=init_energy)
...
def test_merger_elem_w_coupler(
    lattice, tws0, method, parameter=None, update_ref_values=False
):
    """R maxtrix calculation test"""
    d = Drift(l=0.5)
    ...
    R = lattice_transfer_map(lat, energy=init_energy)
    new_lat = merger(
        lat, remaining_types=[], remaining_elems=[cor], init_energy=init_energy
    )
    R_new = lattice_transfer_map(new_lat, energy=init_energy)
...
def test_merger_type(lattice, tws0, method, parameter=None, update_ref_values=False):
    ...
    R = lattice_transfer_map(lat, energy=init_energy)
    new_lat = merger(
        lat, remaining_types=[Drift], remaining_elems=[sol], init_energy=init_energy
    )
    R_new = lattice_transfer_map(new_lat, energy=init_energy)
...
def test_merger_extensive(
    ...
    new_lat = merger(
        lattice,
        remaining_types=[Hcor, Vcor, Monitor],
        remaining_elems=[MPBPMF_47_I1, START_96_I1],
...
def test_merger_tilt(lattice, tws0, method, parameter=None, update_ref_values=False):
    """R maxtrix calculation test"""
    d = Drift(l=0.5)
    q = Quadrupole(l=0.3, k1=3, k2=3.3, eid="quad", tilt=1.0)
    b = Bend(l=1.2, angle=0.02, k1=-1, k2=0.9, tilt=0.2, eid="bend")
    s = Sextupole(l=0.23, k2=22, eid="sext", tilt=2.0)
    c = Cavity(l=2, v=0.02, freq=1.3e9, phi=10, eid="cav")
    cor = Hcor(l=0.1, eid="cor")
    sol = Solenoid(l=0.12, k=0.002)
    d2 = Drift(l=0.5)
    ...
    m = Monitor(eid="mon")
    mat = Matrix(
        l=0.5, r11=1.1, r22=1, r33=1, r44=1, r55=1, r66=1, r12=0.1, t111=0.8, tilt=0.5
    )
    b2 = RBend(l=1.2, angle=0.01, k1=-1, k2=-0.9, eid="bend")
    b3 = SBend(l=1.2, angle=0.02, k1=1, k2=0.5, eid="bend")
...
def test_matrix_write_read(
    ...
    result3 = check_matrix(
        np.array([lat.totalLen]),
        np.array([lat2.totalLen]),
        TOL,
        assert_info="t_matrix - ",
    )
...
def test_matrix_b_vector(
    ...
    result3 = check_matrix(
        ...
        tolerance=1.0e-12,
        tolerance_type="absolute",
        assert_info=" b_vector - ",
    )
    assert check_result(result + result2 + result3)


def test_matrix_b_vector_read_write(
    lattice, tws0, method, parameter=None, update_ref_values=False
):
    """R maxtrix calculation test"""
    d = Drift(l=0.5)
