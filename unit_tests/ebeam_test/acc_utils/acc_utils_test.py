...
def test_rf2beam(lattice, p_array, parameter=None, update_ref_values=False):
    ...
    v1_r, phi1_r, vh_r, phih_r = beam2rf(
        E1,
        chirp,
        curvature,
        skewness,
    ...
    r1 = check_value(
        v1_r, v1, tolerance=1.0e-8, tolerance_type="relative", assert_info="v1"
    )
    r2 = check_value(
        phi1_r, phi1, tolerance=1.0e-8, tolerance_type="relative", assert_info="phi1"
    )
    r3 = check_value(
        vh_r, vh, tolerance=1.0e-8, tolerance_type="relative", assert_info="vh"
    )
    r4 = check_value(
        phih_r, phih, tolerance=1.0e-8, tolerance_type="relative", assert_info="phih"
    )
