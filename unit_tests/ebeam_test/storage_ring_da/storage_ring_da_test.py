...
def test_natural_chromaticity(lattice, tws, update_ref_values=False):
    ...
    result = check_matrix(
        natural_ksi, natural_ksi_ref, TOL, assert_info=" natural_chromaticity - "
    )
    assert check_result(result)
...
def test_compensate_chromaticity(lattice, tws, update_ref_values=False):
    ...
    result1 = check_value(
        ksi[0], ksi_x, match_tolerance, "absotute", assert_info=" ksi_x - \n"
    )
    result2 = check_value(
        ksi[1], ksi_y, match_tolerance, "absotute", assert_info=" ksi_y - \n"
    )
...
def compensate_chromaticity_wrapper(lattice):
    ...
    if not hasattr(pytest, "srda_comp_chromaticity"):
        compensate_chromaticity(
            lattice, ksi_x_comp=ksi_x, ksi_y_comp=ksi_y, nsuperperiod=nsuperperiod
        )
...
def track_nturns_wrapper(lattice):
    ...
    if not hasattr(pytest, "srda_istracked"):
        ...
        pytest.srda_pxy_list = track_nturns(
            lattice,
            nturns,
            pxy_list,
