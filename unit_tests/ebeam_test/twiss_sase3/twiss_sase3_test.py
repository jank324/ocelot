...
def test_lattice_transfer_map_before_rematch(
    lattice, beam=None, update_ref_values=False
):
    """R maxtrix test"""
    ...
    result = check_matrix(
        r_matrix, r_matrix_ref, TOL, assert_info=" r_matrix before_rematch - "
    )
...
def test_lattice_transfer_map_after_rematch(lattice, beam, update_ref_values=False):
    ...
    result = check_matrix(
        r_matrix, r_matrix_ref, TOL, assert_info=" r_matrix after_rematch - "
    )
    assert check_result(result)
...
def rematch(beta_mean, l_fodo, qdh, lat, extra_fodo, beam, qf, qd):
    ...
    m1 = TransferMap(
        create_tm_param_func=create_params,
        delta_e_func=delta_e,
