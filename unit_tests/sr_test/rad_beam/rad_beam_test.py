...
@pytest.mark.parametrize("parameter", [0, 1, 2])
def test_coherent_radiation_traj(
    lattice, screen, beam, parameter, update_ref_values=False
):
    """calculate_radiation function test"""
    screen.nullify()
    accuracy = 1
    ...
    if update_ref_values:
        return {
            "x0": list(screen.beam_traj.x(0)[::10]),
            "y0": list(screen.beam_traj.y(0)[::10]),
            "xp0": list(screen.beam_traj.xp(0)[::10]),
            "yp0": list(screen.beam_traj.yp(0)[::10]),
            "z0": list(screen.beam_traj.z(0)[::10]),
            "s0": list(screen.beam_traj.s(0)[::10]),
            "x1": list(screen.beam_traj.x(1)[::10]),
            "x2": list(screen.beam_traj.x(2)[::10]),
        }
    ...
    result1 = check_matrix(
        screen.beam_traj.x(0)[::10], screen_ref["x0"], TOL, assert_info=" x - "
    )
    result2 = check_matrix(
        screen.beam_traj.y(0)[::10], screen_ref["y0"], TOL, assert_info=" y - "
    )
    result3 = check_matrix(
        screen.beam_traj.xp(0)[::10], screen_ref["xp0"], TOL, assert_info=" xp - "
    )
    result4 = check_matrix(
        screen.beam_traj.yp(0)[::10], screen_ref["yp0"], TOL, assert_info=" yp - "
    )
    result5 = check_matrix(
        screen.beam_traj.z(0)[::10], screen_ref["z0"], TOL, assert_info=" z - "
    )
    result6 = check_matrix(
        screen.beam_traj.s(0)[::10], screen_ref["s0"], TOL, assert_info=" s - "
    )
    result7 = check_matrix(
        screen.beam_traj.x(1)[::10], screen_ref["x1"], TOL, assert_info=" x1 - "
    )
    result8 = check_matrix(
        screen.beam_traj.x(2)[::10], screen_ref["x2"], TOL, assert_info=" x2 - "
    )
    assert check_result(
...
@pytest.mark.parametrize("parameter", [0, 1, 2])
def test_coherent_radiation_fields(
    lattice, screen, beam, parameter, update_ref_values=False
):
    ...
    if update_ref_values:
        return {
            "arReEx": screen.arReEx.tolist(),
            "arImEx": screen.arImEx.tolist(),
            "arReEy": screen.arReEy.tolist(),
            "arImEy": screen.arImEy.tolist(),
            "arPhase": screen.arPhase.tolist(),
        }
    ...
    result1 = check_matrix(
        screen.arReEx, screen_ref["arReEx"], TOL, assert_info=" arReEx - "
    )
    result2 = check_matrix(
        screen.arImEx, screen_ref["arImEx"], TOL, assert_info=" arImEx - "
    )
    result3 = check_matrix(
        screen.arReEy, screen_ref["arReEy"], TOL, assert_info=" arReEy - "
    )
    result4 = check_matrix(
        screen.arImEy, screen_ref["arImEy"], TOL, assert_info=" arImEy - "
    )
    result5 = check_matrix(
        screen.arPhase, screen_ref["arPhase"], TOL, assert_info=" arPhase - "
    )
    assert check_result(result1 + result2 + result3 + result4 + result5)


@pytest.mark.parametrize("parameter", [0, 1, 2])
def test_coherent_radiation_parray(
    lattice, screen, beam, parameter, update_ref_values=False
):
    """calculate_radiation function test"""
    screen.nullify()
...
def test_energy_loss_spatial(
    lattice, screen, beam, parameter=None, update_ref_values=False
):
    """calculate_radiation fucntion test"""
    beam = Beam()
    beam.E = 17.5
    ...
    screen = calculate_radiation(
        lat, copy.deepcopy(screen), beam, energy_loss=True, accuracy=1
    )
