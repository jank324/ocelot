...
def test_segments(lattice, screen, beam, update_ref_values=False):
    ...
    result1 = check_matrix(
        screen_segm.Eph, screen_whole.Eph, TOL, assert_info=" Eph - "
    )
    result2 = check_matrix(
        screen_segm.Yph, screen_whole.Yph, TOL, assert_info=" Yph - "
    )
    result3 = check_matrix(
        screen_segm.Xph, screen_whole.Xph, TOL, assert_info=" Xph - "
    )
    result4 = check_matrix(
        screen_segm.Total, screen_whole.Total, TOL, assert_info=" Total - "
    )
    result5 = check_matrix(
        screen_segm.Sigma, screen_whole.Sigma, TOL, assert_info=" Sigma - "
    )
    result6 = check_matrix(
        screen_segm.Pi, screen_whole.Pi, tolerance=1.0e-6, assert_info=" Pi - "
    )
    assert check_result(result1 + result2 + result3 + result4 + result5 + result6)
