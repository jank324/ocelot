...
def bm_e(x, y, xp, yp, x0, y0, z, p_en, R):
    ...
    phi_s = p_en / hc * z * (th_x**2 + th_y**2) / (2.0)
    phi_0 = -p_en / (2.0 * hc) * R * (th_x - xp) * (
        1.0 / gam**2 + (th_y - yp) ** 2 + (th_x - xp) ** 2 / 3.0
    ) - p_en / hc * (x * th_x + y * th_y)
...
def bm_e_a(x, y, xp, yp, x0, y0, z, p_en, R):
    ...
    for i in range(len(x)):
        for j in range(len(xp)):
            phi_0 = -p_en / (2.0 * hc) * R * (th_x - xp[j]) * (
                1.0 / gam**2 + (th_y - yp) ** 2 + (th_x - xp[j]) ** 2 / 3.0
            ) - p_en / hc * (x[i] * xp[j] + y * yp)
            Ex += (
                np.exp(1j * phi_s)
                * np.exp(1j * phi_0)
                * Ex0
                * exp(-0.5 * (x[i] / sigx) ** 2 - 0.5 * (xp[j] / sigxp) ** 2)
                / (sigx * sigxp)
            )
