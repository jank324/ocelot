...
class Mask(Grid):
    ...
    def __mul__(self, other):  # check this stuff when it will been needed
        ...
        if (
            ...
            ValueError("'other' must belong to Mask class")
...
class PropMask(Mask):
    ...
    def apply(self, dfl):
        ...
        if self.method in ["PropMask", "PropMask_kf"]:
            ...
            raise ValueError(
                "propagation method should be 'PropMask', 'PropMask_kf' or 'PropMask_kt'"
            )
    ...
    def get_mask(self, dfl):
        ...
        if dfl.domain_z == "f":
            ...
            for i in range(dfl.Nz()):
                self.msk[i, :, :] *= np.exp(
                    1j * self.z0 * (np.sqrt(k[i] ** 2 - k_x**2 - k_y**2) - k[i])
                )
            ...
            self.msk = np.exp(
                1j * self.z0 * (np.sqrt(k**2 - k_x**2 - k_y**2) - k)
            )
        ...
        _logger.debug(
            ind_str + "transfer function mask shape {}".format(self.msk.shape)
        )
        return self.msk
...
class Prop_mMask(Mask):
    ...
    def get_mask(self, dfl):
        ...
        if dfl.domain_z == "f":
            ...
            if self.mx != 0:
                self.msk *= np.exp(
                    1j * self.z0 / self.mx * (np.sqrt(k**2 - k_x**2) - k)
                )  # for i in range(dfl.Nz())]
            if self.my != 0:
                self.msk *= np.exp(
                    1j * self.z0 / self.my * (np.sqrt(k**2 - k_y**2) - k)
                )  # for i in range(dfl.Nz())]
        else:
            raise ValueError("wrong field domain, domain must be 'ks' or 'kf'")
    ...
    def apply(self, dfl):
        ...
        if self.mx != 1:
            ...
            dfl = QuadCurvMask(r=-self.mx * self.z0 / (self.mx - 1), plane="x").apply(
                dfl
            )
        if self.my != 1:
            #            dfl.curve_wavefront(-self.my * self.z0 / (self.my - 1), plane='y')
            dfl = QuadCurvMask(r=-self.my * self.z0 / (self.my - 1), plane="y").apply(
                dfl
            )
...
class QuadCurvMask(Mask):
    ...
    def apply(self, dfl):
        ...
        _logger.debug(
            "curving radiation wavefront by {}m in {} domain".format(
                self.r, self.domain_z
            )
        )
...
class PhaseDelayMask(Mask):
    ...
    def get_mask(self, dfl):
        ...
        coeff_norm = [
            ci / (1e15) ** i / factorial(i) for i, ci in enumerate(self.coeff)
        ]
...
class HeightErrorMask_1D(Mask):
    ...
    def __init__(
        self, height_profile=None, hrms=0, axis="x", angle=np.pi * 2 / 180, seed=None
    ):
        Mask.__init__(self)
        # self.mask = None
    ...
    def get_mask(self, dfl):
        ...
        if self.height_profile is None:
            ...
            self.height_profile = self.height_profile.generate_1d_profile(
                self.hrms, L=eff_l, N=n, seed=self.seed
            )
            ...
            _logger.info(
                ind_str
                + "given height_profile was interpolated to length and ".format(
                    dfl.shape()
                )
            )
        ...
        phase_delay = (
            2 * 2 * np.pi * np.sin(self.angle) * self.height_profile.h / dfl.xlamds
        )
...
class MirrorMask(Mask):
    ...
    def get_mask(self, dfl):
        ...
        if self.msk is None:
            heightErr_x = HeightErrorMask_1D(
            ...
            heightErr_y = HeightErrorMask_1D(
                height_profile=self.height_profile,
                hrms=self.hrms,
                axis="y",
                angle=self.angle,
