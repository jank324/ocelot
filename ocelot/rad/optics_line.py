...
class OpticsLine:
    ...
    def get_sequence_part(self, start, stop):
        try:
            ...
            _logger.info(
                ind_str
                + "sequence the first element {} and its index {}".format(
                    self.start, id1
                )
            )
            ...
            if stop is not None:
                ...
                _logger.info(
                    ind_str
                    + "sequence the last element {} and its index {}".format(
                        self.start, id2
                    )
                )
            else:
    ...
    if element.__class__ is None:
        ...
        if element.method in ["PropMask", "PropMask_kf", "PropMask_kt"]:
            if element.mx == 1 and element.my == 1:
                ...
                element.mask = Prop_mMask(
                    element.l, element.mx, element.my, element.method
                )
            else:
            ...
            raise ValueError(
                "Propagator method can be PropMask (see Tutorials), Fraunhofer_Propagator or Fresnel_Propagator"
            )
        ...
        element.mask = ApertureEllipsMask(
            element.ax, element.ay, element.cx, element.cy
        )
        ...
        element.mask = MirrorMask(
            element.height_profile,
            element.hrms,
            element.angle,
            element.plane,
            element.lx,
        ...
        raise ValueError(
            "Optics element must belong to one of the child OpticsElement classes"
        )
