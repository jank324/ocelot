...
def Ephoton2K(Eph, lu=0.04, Eeb=14):
    gamma = Eeb / m_e_GeV
    K = np.sqrt(4.0 * gamma * gamma / lu * h_eV_s * speed_of_light / Eph - 2)
    return K


def K2Ephoton(K, lu=0.04, E=14):
    gamma = E / m_e_GeV
    Eph = 4.0 * gamma * gamma * h_eV_s * speed_of_light / ((K * K + 2) * lu)
    return Eph
...
def field2K(field, lu=0.04):
    ...
    :return: K
    """
    K = field * lu * speed_of_light / (m_e_eV * 2.0 * pi)
    return K


def K2field(K, lu=0.04):
    field = K * m_e_eV * 2.0 * pi / (lu * speed_of_light)
    return field
...
def field2Ephoton(field, lu=0.04, E=14):
    gamma = E / m_e_GeV
    K = field2K(field, lu)
    l = lu / (2.0 * gamma * gamma) * (1.0 + K * K / 2.0)
...
def Ephoton2field(energy, lu=0.04, Eeb=14):
    ...
    field = K * 2.0 * pi * m_e_eV / (lu * speed_of_light)
    return field


def lambda2Ebeam(Lambda=10.4e-9, lu=0.0272, K=1.2392):
    gamma = np.sqrt(lu / (2.0 * Lambda) * (1.0 + K * K / 2.0))
...
class ID_radiation:
    ...
    def eff_sizes(self, K):
        ...
        L = self.undul.l
        self.sigma_r = np.sqrt(Lambda * L / (2 * 4.0 * pi * pi))
        self.sigma_r1 = np.sqrt(Lambda / L / 2.0)
...
def print_rad_props(beam, K, lu, L, distance):
    ...
    def f_n(n, Ku):
        v1 = (n - 1) / 2.0
        v2 = (n + 1) / 2.0
        x = n * Ku * Ku / (4.0 + 2.0 * Ku * Ku)
        return (
            n * n * Ku * Ku / ((1 + Ku * Ku / 2.0) ** 2) * (jn(v1, x) - jn(v2, x)) ** 2
        )
    ...
    sigma_r = np.sqrt(Lambda * L / (2 * 4.0 * pi * pi))
    sigma_r1 = np.sqrt(Lambda / L / 2.0)
    ...
    F = flux(beam.I, K, m=1)
    N = L / lu
    flux_tot = 1.431e14 * beam.I * N * f_n(1, K) * (1.0 + K * K / 2.0) / 1.0 / 2.0

    brightness = (
        flux_tot / (4 * pi * pi * Sigma_x * Sigma_y * Sigma_x1 * Sigma_y1) * 1e-12
    )
    ...
    print("1/gamma      : ", np.round(1.0 / gamma * 1e6, 4), " um")
    ...
    print(
        "H. spot size : ",
        np.round(size_x * 1000.0, 4),
        "/",
        np.round(size_x / distance * 1000.0, 4),
        " mm/mrad",
    )
    print(
        "V. spot size : ",
        np.round(size_y * 1000.0, 4),
        "/",
        np.round(size_y / distance * 1000.0, 4),
        " mm/mrad",
    )
    print("I            : ", beam.I, " A")
    print("Nperiods     : ", L / lu)
    ...
    print(
        "flux density : ",
        "{0:.2E}".format(F),
        " ph/sec/mrad^2/0.1%BW;   ",
        "{0:.2E}".format(F / distance / distance),
        " ph/sec/mm^2/0.1%BW",
    )
...
class UndulatorParameters:
    ...
    def recalculate(self):
        self.gamma = 1.0e9 * self.E / m_e_eV
    ...
    def computeRadiationAnalytical(self, z0, theta, phi, dwRel=0.1, npoints=100):
        ...
        u = (
            ...
            * self.K
            * self.K
            / (8 * self.kw * speed_of_light * self.gamma * self.gamma)
        )
        ...
        I = (
            ...
            * np.exp(1j * phis)
            / (z0 * self.gamma)
            * np.sinc(pi * self.Nw * (w - w1) / w1)
            * ajj
        )
        I = np.real(I)

        self.fundamentalWavelength = (
            self.lw
            / (2 * self.gamma**2)
            * (1 + self.K**2 / 2.0 + self.gamma**2 * theta)
        )
    ...
    def get_k(self, E):
        # w1 in ev
        w1 = 2.0 * np.pi * E / h_eV_s
        return np.sqrt((4.0 * self.kw * speed_of_light * self.gamma**2) / w1 - 2.0)
