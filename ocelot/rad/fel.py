...
class FelParameters:
    ...
    def eval(self, method="mxie"):
        ...
        self.lambda0 = (
            self.xlamd / (2.0 * self.gamma0**2) * (1.0 + self.aw0**2)
        )  # resonant wavelength
        if np.any(self.lambda0 < 0):
            _logger.error(
                "wavelength is not reachable with und_period {} gamma {} and K {}".format(
                    self.xlamd, self.gamma0, self.aw0
                )
            )
            self.inaccurate = True
        ...
        self.emit_nn = (
            2 * np.pi * emit_n / self.lambdah / self.gamma0
        )  ## emittance normalized as in Eq.6, 10.1103/PhysRevSTAB.15.080702
        if np.any(self.emit_nn < 1) or np.any(self.emit_nn) > 5:
            self.inaccurate = True
            if tdp:
                _logger.warning(
                    "1 <! min(emittance) {} <! 5, SSY approx. might be incorrect".format(
                        np.nanmin(self.emit_nn)
                    )
                )
            else:
                _logger.warning(
                    "1 <! emittance {} <! 5, SSY approx. might be incorrect".format(
                        self.emit_nn
                    )
                )
        ...
        if method == "mxie":
            ...
            self.rho1 = (0.5 / self.gamma0) * np.power(
                (self.aw0 * self.fc * self.xlamd / 2 / np.pi) ** 2
                / (self.rxbeam * self.rybeam)
                * self.I
                / I_Alfven,
                1.0 / 3.0,
            )
            ...
            self.lg1 = self.xlamd / (
                4 * np.pi * np.sqrt(3) * self.rho1
            )  # power gain length [Xie]
            ...
            self.xie_etae = (
                ...
                * np.pi
                * self.lg1
                * (self.emitx * self.emity)
                / self.lambda0
                / (self.rxbeam * self.rybeam)
                / self.gamma0**2
                * ((self.fc / self.fch) ** 2 / self.hn) ** (1 / 3)
                / self.hn
            )  # expressed via average x-y beam size
            self.xie_etagamma = self.deta / (self.rho1 * np.sqrt(3))

            if self.hn != 1:
                self.xie_etad *= ((self.fc / self.fch) ** 2 / self.hn) ** (
                    1 / 3
                ) / self.hn
                ...
                self.xie_etagamma *= ((self.fc / self.fch) ** 2 / self.hn) ** (
                    1 / 3
                ) * self.hn  # eq C2+ DOI:10.1103/PhysRevSTAB.15.080702

            self.delta = (
                a[1] * self.xie_etad ** a[2]
                + a[3] * self.xie_etae ** a[4]
                + a[5] * self.xie_etagamma ** a[6]
                + a[7] * self.xie_etae ** a[8] * self.xie_etagamma ** a[9]
                + a[10] * self.xie_etad ** a[11] * self.xie_etagamma ** a[12]
                + a[13] * self.xie_etad ** a[14] * self.xie_etae ** a[15]
                + a[16]
                * self.xie_etad ** a[17]
                * self.xie_etae ** a[18]
                * self.xie_etagamma ** a[19]
            )
            ...
            self.lg1 = (
                0.5
                * 1.67
                * np.sqrt(I_Alfven / self.I)
                * (emit_n * self.xlamd) ** (5 / 6)
                / self.lambdah ** (2 / 3)
                * (1 + self.aw0**2) ** (1 / 3)
                / (self.hn ** (5 / 6) * self.aw0 * self.fch)
            )
        ...
        if self.qf == 1:
            self.delta_q = (
                ...
                * (1 + self.aw0**2) ** (9 / 4)
                * F_aw
                / (self.aw0 * self.fch**3 * self.hn ** (5 / 3))
            )

            if np.any(self.delta_q >= 1):
                _logger.warning(
                    "quantum fluctuation effect exceeds 1, estimation not applicable anymore"
                )
                self.delta_q = 0.999
                self.inaccurate = True
        else:
        ...
        if np.any(self.delta_eff > self.delta_criterion):
            if tdp:
                _logger.warning(
                    "delta_eff > delta_criterion; SSY approx. might be incorrect"
                )
            else:
                _logger.warning(
                    "delta_eff {} > {}; SSY approx. might be incorrect".format(
                        self.delta_eff, self.delta_criterion
                    )
                )
            self.inaccurate = True
        ...
        self.beta_opt_calc = (
            11.2
            * (I_Alfven / self.I) ** (1 / 2)
            * (emit_n**3 * self.xlamd) ** (1 / 2)
            / (self.lambdah * self.hn ** (1 / 2) * self.aw0 * self.fch)
            / (1 + 8 * self.delta_eff) ** (1 / 3)
        )
        ...
        self.P_sn = (3 * self.rho3 * self.Pb) / (
            self.Nc * np.sqrt(np.pi * np.log(self.Nc))
        )  # shot noise power [W]

        self.z_sat_norm = 3 + 1 / np.sqrt(3) * np.log(
            self.Nc
        )  # normalized saturation length for slices
        self.z_sat_magn = (
            self.z_sat_norm * np.sqrt(3) * self.lg3
        )  # magnetic length to reach saturation
    ...
    def log(self, type="debug"):
        ...
        _log_func(
            "FEL_E_photon   = {} eV".format(h_eV_s * speed_of_light / self.lambda0)
        )
        ...
        if z is None:
            ...
            if np.size(z) > 1:
                ...
                if (z > self.z_sat_min).any():
                    _logger.warning(
                        "Estimation applicable up to z_sat_min=%.2fm, limiting power to saturation level"
                        % (self.z_sat_min)
                    )
                ...
                if z > self.z_sat_min:
                    _logger.warning(
                        "Estimation applicable up to z_sat_min=%.2fm, while z=%.2fm requested, returning saturation power"
                        % (self.z_sat_min, z)
                    )
                    z = self.z_sat_min
    ...
    def tcoh(self, z=None):
        ...
        tcoh = (
            self.lambda0
            / (6 * self.rho3 * speed_of_light)
            * np.sqrt(z / (2 * np.pi * self.lg3))
        )
        return tcoh
    ...
    def spectrogram(self, z=None):
        ...
        for i in range(s_arr.size):
            if dphen[i] != 0:
                spec[i] = np.exp(
                    -((phen_arr - phen0[i]) ** 2) / 2 / dphen[i] ** 2
                ) / np.sqrt(2 * np.pi * dphen[i] ** 2)
