...
class Screen:
    ...
    def __init__(self):
        ...
        self.arReEx = (
            []
        )  # array, Real part of horizontal component of the electric field
        self.arImEx = (
            []
        )  # array, Imaginary part of horizontal component of the electric field
        self.arReEy = (
            []
        )  # array, Real part of the vertical component of the electric field
        self.arImEy = (
            []
        )  # array, Imaginary part of the vertical component of the electric field
    ...
    def update(self):
        self.x_start = (self.x - self.size_x) * 1000.0  # in mm

        if self.nx != 1:
            self.x_step = float(self.size_x * 2.0) / (self.nx - 1) * 1000.0  # in mm
        else:
            self.x_step = 0.0  # in mm
            self.x_start = self.x * 1000.0  # in mm
        # Y
        self.y_start = (self.y - self.size_y) * 1000.0  # in mm
        if self.ny != 1:
            self.y_step = float(self.size_y * 2.0) / (self.ny - 1) * 1000.0  # in mm
        else:
            self.y_step = 0  # in mm
            self.y_start = self.y * 1000.0  # in mm
        # E
        self.e_start = self.start_energy
        if self.num_energy != 1:
            self.e_step = (float(self.end_energy) - self.start_energy) / (
                self.num_energy - 1
            )
        else:
            self.e_step = 0
        ...
        self.Zstart = 0.0  # this parameter is needed to calculation radiation

        self.Distance = (
            self.z * 1000.0
        )  # Note this parameter is just distance from ZERO to screen # in mm!!!
        ...
        self.Xph = np.linspace(
            self.x_start, self.x_start + self.x_step * (self.nx - 1), self.nx
        )
        self.Yph = np.linspace(
            self.y_start, self.y_start + self.y_step * (self.ny - 1), self.ny
        )
        self.Eph = np.linspace(
            self.e_start, self.e_start + self.e_step * (self.ne - 1), self.ne
        )
    ...
    def rebuild_efields(self, x0=0, y0=0, z0=0):
        ...
        Xscr = np.linspace(
            self.x_start, self.x_start + self.x_step * (self.nx - 1), num=self.nx
        )
        Yscr = np.linspace(
            self.y_start, self.y_start + self.y_step * (self.ny - 1), num=self.ny
        )
        Yscr = Yscr.reshape((self.ny, 1))
        Erad = np.linspace(
            self.e_start, self.e_start + self.e_step * (self.ne - 1), num=self.ne
        )
        ...
        phaseConstIn = (
            ...
            * Erad
            / hc
            * (prXconst * prXconst + prYconst * prYconst)
            / (self.Distance - z0)
        )
    ...
    def screen_to_emscreen(self, screen):
        ...
        # X
        self.x_start = (screen.x - screen.size_x) * 1000.0  # in mm

        if screen.nx != 1:
            self.x_step = float(screen.size_x * 2.0) / (screen.nx - 1) * 1000.0  # in mm
        else:
            self.x_step = 0.0
            self.x_start = screen.x * 1000.0  # in mm
        self.nx = screen.nx  # points per horizontal direction
        # Y
        self.y_start = (screen.y - screen.size_y) * 1000.0  # in mm
        if screen.ny != 1:
            self.y_step = float(screen.size_y * 2.0) / (screen.ny - 1) * 1000.0  # in mm
        else:
            self.y_step = 0
            self.y_start = screen.y * 1000.0  # in mm
        self.ny = screen.ny  # points per vertical direction
        # E
        self.e_start = screen.start_energy
        if screen.num_energy != 1:
            self.e_step = (float(screen.end_energy) - screen.start_energy) / (
                screen.num_energy - 1
            )
        else:
            self.e_step = 0
    ...
    def screenPy2C(self, lperiod, nperiods, status):
        ...
        scrPrm[11] = lperiod * 1000.0
        scrPrm[12] = nperiods
        scrPrm[
            13
        ] = status  # status is just for fast mode calculation (ideal undulator and zero initial conditions)
...
def sum_screens(screen_down, screen_up):
    ...
    screen.arReEx = (
        screen_down.arReEx + screen_up.arReEx * cosfa - screen_up.arImEx * sinfa
    )
    screen.arImEx = (
        screen_down.arImEx + screen_up.arImEx * cosfa + screen_up.arReEx * sinfa
    )
    screen.arReEy = (
        screen_down.arReEy + screen_up.arReEy * cosfa - screen_up.arImEy * sinfa
    )
    screen.arImEy = (
        screen_down.arImEy + screen_up.arImEy * cosfa + screen_up.arReEy * sinfa
    )
