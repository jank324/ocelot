...
def derivat(x, y):
    ...
    for i in range(2):
        ...
        derivative = (
            3.0 * a0 * (x[N * i] - x[n * i]) * (x[N * i] - x[n * i])
            + 2.0 * b0 * (x[N * i] - x[n * i])
        ) * i + c0
        if i == 0:
            deriv1 = derivative
...
def moment(x, y):
    ...
    for i in range(1, N):
        h1 = x[i + 1] - x[i]
        F[i] = 6.0 * ((y[i + 1] - y[i]) / h1 - (y[i] - y[i - 1]) / h0)
        k = h0 * Alfa[i] + 2.0 * (h1 + h0)
        Alfa[i + 1] = -h1 / k
        Beta[i + 1] = (F[i] - h0 * Beta[i]) / k
        h0 = h1

    F[N] = 6.0 * (deriv2 - (y[N] - y[N - 1]) / h0)
    M = np.zeros(len(x))
    M[N] = (F[N] - h0 * Beta[N]) / (h0 * Alfa[N] + 2.0 * h0)
...
def cspline_coef(x, y):
    ...
    for i in range(n - 1):
        ...
        a[i] = (M[i + 1] - M0) / (6.0 * h)
        b[i] = M0 / 2.0
        c[i] = (y[i + 1] - y[i]) / h - M[i + 1] * h / 6.0 - M0 * h / 3.0
        d[i] = y[i]
        z[i] = x[i]
    return a, b, c, d, z
...
if __name__ == "__main__":
    ...
    plot(x, x / 2.0 - 1.0 / 4.0 * sin(2 * x), x, y2)
    show()
