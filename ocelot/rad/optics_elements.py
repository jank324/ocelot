...
class FreeSpace(OpticsElement):
    ...
    def __init__(self, l=0.0, mx=1, my=1, method="PropMask", eid=None):
...
class ApertureRect(Aperture):
    ...
    def __init__(self, lx=np.inf, ly=np.inf, cx=0.0, cy=0.0, eid=None):
...
class ApertureEllips(Aperture):
    ...
    def __init__(self, ax=np.inf, ay=np.inf, cx=0.0, cy=0.0, eid=None):
        Aperture.__init__(self, eid=eid)
        self.ax = ax
        self.ay = ay
        self.cx = cx
