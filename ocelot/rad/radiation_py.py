...
from ocelot.cpbd import beam, optics, track
...
class BeamTraject:
    ...
    def s(self, n=0):
        ...
        s = cumtrapz(np.sqrt(1.0 + xp2 + yp2), self.z(n), initial=0)
        return s
...
def x2xgaus(X):
    ...
    :return: new array
    """
    sqrt35 = 0.5 * np.sqrt(3.0 / 5.0)
    xnew = [X[0]]
    h = X[1] - X[0]
    xgaus = (
        np.array([0.5 - sqrt35, 0.5 - sqrt35 + sqrt35, 0.5 - sqrt35 + sqrt35 + sqrt35])
        * h
    )
    for x in X[:-1]:
...
def traj2motion(traj):
    ...
    motion.x = bspline(motion.z, motion.x, Z) * 1000.0
    motion.y = bspline(motion.z, motion.y, Z) * 1000.0
    # print "inegr = ", simps(motion.bx.flatten()**2, motion.z*1000)
    Ibx2 = integ_beta2(motion.z * 1000.0, motion.bx)
    Iby2 = integ_beta2(motion.z * 1000.0, motion.by)
...
def sigma_gamma_quat(energy, Kx, lperiod, L):
    ...
    def f(K):
        return 1.2 + 1.0 / (K + 1.33 * K * K + 0.4 * K**3)
...
def quantum_diffusion(energy, Kx, lperiod, L, quantum_diff=False):
    if quantum_diff:
        ...
        sigma_Eq = sigma_gamma_quat(
            energy, Kx, lperiod, L
        )  # sqrt(delta_Eq2/(gamma*gamma))
...
def gintegrator(Xscr, Yscr, Erad, motion, screen, n, n_end, gamma, half_step):
    ...
    for p in range(3):  # // Gauss integration
        ...
        radConst = (
            ...
            * (screen.Distance)
            / LenPntrZ
            / ((1.0 + tx2 + ty2) * (1.0 + tx2 + ty2))
        )

        radX = radConst * (
            By * (1.0 - tx2 + ty2) + Bx * tyx - 2.0 * tx / Q / LenPntrZ
        )  # /*sigma*/
        radY = -radConst * (
            Bx * (1.0 + tx2 - ty2) + By * tyx + 2.0 * ty / Q / LenPntrZ
        )  # ;/*pi*/
        ...
        phase = (
            ...
            * (
                ZZ
                - motion.z[0]
                + gamma2 * (IbetX2 + IbetY2 + phaseConstCur - phaseConstIn)
            )
            + screen.arPhase
        )
...
def gintegrator_over_traj_py(
    ...
    for n in range(Nmotion - 1):
        for p in range(3):  # // Gauss integration
            ...
            radX = radConst * (
                By[i] * (1.0 - tx2 + ty2) + Bx[i] * tyx - 2.0 * tx / q / LenPntrZ
            )  # /*sigma*/
            radY = -radConst * (
                Bx[i] * (1.0 + tx2 - ty2) + By[i] * tyx + 2.0 * ty / q / LenPntrZ
            )  # ;/*pi*/
            ...
            phase = (
                ...
                * (
                    z[i]
                    - z[0]
                    + gamma2 * (XbetaI2[i] + YbetaI2[i] + phaseConstCur - phaseConstIn)
                )
                + arPhase
            )
...
gintegrator_over_traj = (
    gintegrator_over_traj_py if not nb_flag else nb.jit(gintegrator_over_traj_py)
)


def wrap_gintegrator(
    Nmotion, Xscr, Yscr, Erad, motion, screen, n_end, gamma, half_step
):
...
def radiation_py(gamma, traj, screen):
    ...
    half_step = (motion.z[-1] - motion.z[0]) / 2.0 / (Nmotion - 1)

    n_end = len(motion.z) - 2
    Xscr = np.linspace(
        screen.x_start, screen.x_start + screen.x_step * (screen.nx - 1), num=screen.nx
    )
    Yscr = np.linspace(
        screen.y_start, screen.y_start + screen.y_step * (screen.ny - 1), num=screen.ny
    )
    Yscr = Yscr.reshape((screen.ny, 1))
    Erad = np.linspace(
        screen.e_start, screen.e_start + screen.e_step * (screen.ne - 1), num=screen.ne
    )
    ...
    if 1 in shape_array:
        ...
        for i, erad in enumerate(Erad):
            ...
            screen.arPhase[
                i * n_pl : (i + 1) * n_pl
            ] = screen_segment.arPhase.flatten()[:]
...
def track4rad_beam(
    p_array, lat, energy_loss=False, quantum_diff=False, accuracy=1, end_poles=False
):
    """
        ...
        if elem.__class__ != Undulator:
            ...
            if elem.mag_field is not None:
                ...
                if len(elem.field_map.z_arr) != 0:
                    ...
                    z_array = (
                        elem.field_map.z_arr - elem.field_map.z_arr[0]
                    ) * unit_coef
                    mag_field = field_map2field_func(
                        z=z_array, By=elem.field_map.By_arr
                    )
                else:
                    ...
                    def mag_field(x, y, z):
                        return und_field(
                            x, y, z, elem.lperiod, elem.Kx, nperiods=nperiods
                        )
...
if __name__ == "__main__":
    quantum_diffusion(17.5, 4.0, 0.04, 200.0, quantum_diff=True)
    x = np.linspace(0, 1, 4)
    xnew = x2xgaus(x)
    print(x)
    print(xnew)
