...
m_e_MeV = m_e_eV / 1e6  # MeV (0.510998928)
m_e_GeV = m_e_eV / 1e9  # GeV

mu_0 = 4 * pi * 1e-7  # permeability of free space (1.2566370614359173e-06)
epsilon_0 = (
    1 / mu_0 / speed_of_light**2
)  # permittivity of free space (8.854187817620e-12 F/m)

h_eV_s = h_J_s / q_e  # [eV*s]
hr_eV_s = h_eV_s / 2.0 / pi
ro_e = q_e**2 / (
    4 * pi * epsilon_0 * m_e_kg * speed_of_light**2
...
I_Alfven = (
    4 * np.pi * epsilon_0 * m_e_eV * speed_of_light
)  # Alfven (Budker) current [A], ~17kA

Cgamma = 4.0 * pi / 3.0 * ro_e / m_e_MeV**3
Cq = 55.0 / (32.0 * np.sqrt(3) * 2 * pi) * h_eV_s * speed_of_light / m_e_eV

Z0 = 1.0 / (speed_of_light * epsilon_0)  # Ohm - impedance of free space
