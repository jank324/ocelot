...
def conj_sym(x):
    ...
    x[int(np.ceil(n / 2) + c) : n] = (
        x_part_flipped.real - np.sqrt(-1 + 0j) * x_part_flipped.imag
    )
...
def gs_search(f, bracket, tol=1.0e-6, nmax=50):
    """
...
def fit_gauss_2d(x, y, F):
    ...
    for i in range(n1):
        sig1 += (x[i] - mu1) ** 2 * sum(F[i, :])
    sig1 /= sum(F[:, :])

    sig2 = 0

    for i in range(n2):
        sig2 += (y[i] - mu2) ** 2 * sum(F[:, i])
    sig2 /= sum(F[:, :])
...
corr_c_nb = (
    nb.jit("void(double[:,:], int32, double[:,:], int32)", nopython=True, nogil=True)(
        corr_c_py
    )
    if numba_avail
    else corr_f_np
...
mut_coh_func = (
    nb.jit(
        "void(complex128[:,:,:,:], complex128[:,:,:], int32)", nopython=True, nogil=True
    )(mut_coh_func_py)
    if numba_avail
    else mut_coh_func_py
)
