...
from numpy import array, exp, meshgrid, pi, reshape, shape, sqrt, zeros

from ocelot.lib.genera.src.python.convolution.convolution_gauss import (
    convolution_1D_cpp,
    convolution_2D_cpp,
)
...
def beam_sizes_on_screen(beam, screen):
    if beam.emit_x == 0 or beam.emit_y == 0:
        ...
        return False
    beam.sizes()
    screen.beam_size_x = sqrt(
        beam.sigma_x * beam.sigma_x * 1e6 + (beam.sigma_xp * screen.Distance) ** 2
    )  # mm
    screen.beam_size_y = sqrt(
        beam.sigma_y * beam.sigma_y * 1e6 + (beam.sigma_yp * screen.Distance) ** 2
    )  # mm
...
def change_sizes_screen(screen, beam):
    ...
    screen.sigma_e = 2.0 * beam.sigma_E / beam.E * screen.start_energy
    ...
    if beam_sizes_on_screen(beam, screen):
        if screen.ne == 1:
            screen.nx, screen.x_step, screen.x_start, screen.nx_add = change_size(
                screen.nx, screen.x_step, screen.x_start, screen.beam_size_x * 1.0
            )  # emittance X
            screen.ny, screen.y_step, screen.y_start, screen.ny_add = change_size(
                screen.ny, screen.y_step, screen.y_start, screen.beam_size_y * 1.0
            )  # emittance Y
            screen.ne, screen.e_step, screen.e_start, screen.ne_add = change_size(
                screen.ne, screen.e_step, screen.e_start, screen.sigma_e * 3.0
            )  # energy spread
            ...
            if screen.theta_x != 0 and screen.theta_y != 0:
                screen.nx, screen.x_step, screen.x_start, screen.nx_add = change_size(
                    ...
                    screen.x_step,
                    screen.x_start,
                    screen.theta_x * screen.Distance,
                )  # emittance X
                screen.ny, screen.y_step, screen.y_start, screen.ny_add = change_size(
                    screen.ny,
                    screen.y_step,
                    screen.y_start,
                    screen.theta_y * screen.Distance,
                )  # emittance Y
                screen.ne, screen.e_step, screen.e_start, screen.ne_add = change_size(
                    screen.ne, screen.e_step, screen.e_start, screen.sigma_e * 3.0
                )  # energy spread
                ...
                screen.nx, screen.x_step, screen.x_start, screen.nx_add = change_size(
                    screen.nx, screen.x_step, screen.x_start, screen.beam_size_x
                )  # emittance X
                screen.ny, screen.y_step, screen.y_start, screen.ny_add = change_size(
                    screen.ny, screen.y_step, screen.y_start, screen.beam_size_y
                )  # emittance Y
                screen.ne, screen.e_step, screen.e_start, screen.ne_add = change_size(
                    screen.ne, screen.e_step, screen.e_start, screen.sigma_e * 3.0
                )  # energy spread
            ...
            print(
                "Emittance switched off. Screen sizes are wrong. ne >1 and (nx or ny) >1"
            )
        ...
        print(
            "1_ Emittance switched off. Screen sizes are wrong. ne >1 and (nx or ny) >1"
        )
...
def emittance_on(screen):
    ...
    for ie in range(screen.ne):
        ...
        data_pi = data_pi[
            screen.ny_add : -screen.ny_add, screen.nx_add : -screen.nx_add
        ]
...
def spread_on(screen, Pi, Sigma):
    ...
    # screen.fund_harm_eV -> screen.start_energy
    k = lambda energy: exp(
        -((energy - screen.start_energy) ** 2) / (2.0 * sigma_e * sigma_e)
    ) / (sqrt(2.0 * pi) * sigma_e)
...
if __name__ == "__main__":
    ...
    beam = Beam()
    beam = Beam(x=0.0, xp=-0.000115 * 0.0, y=0, yp=0)
    beam.E = 17.5
