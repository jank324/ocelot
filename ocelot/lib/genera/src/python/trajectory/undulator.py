...
import os
from ctypes import CDLL, POINTER, c_double, c_int, cdll
from os import name as os_name
from sys import exit, path

from numpy import array, hstack, linspace, pi, zeros
...
def define_points_undul(undulator, accuracy):
    ...
    if undulator.field_file == None:
        ...
        if undulator.field_map.units == "mm":
            npoints_traj = (
                int(
                    ((undulator.field_map.z_arr[-1] - undulator.field_map.z_arr[0]) * 4)
                    + 10
                )
                * accuracy
            )
            ...
            npoints_traj = (
                int(
                    (
                        (undulator.field_map.z_arr[-1] - undulator.field_map.z_arr[0])
                        * 2000
                    )
                    + 10
                )
...
def field_map_for_dll(undulator):
    ...
    if undulator.field_file != None:
        ...
        if fm.units == "m":
            x_arr = fm.x_arr * 1000.0
            y_arr = fm.y_arr * 1000.0
            z_arr = fm.z_arr * 1000.0
        else:
            x_arr = fm.x_arr
            y_arr = fm.y_arr
            z_arr = fm.z_arr
        ...
        undulator.By = (
            undulator.Kx * m_e_eV * 2.0 * pi / (undulator.lperiod * speed_of_light)
        )  # [T]
        undulator.Bx = (
            undulator.Ky * m_e_eV * 2.0 * pi / (undulator.lperiod * speed_of_light)
        )  # [T}
        undul_param = [
            undulator.Bx,
            undulator.By,
        ...
        fm.l = undulator.lperiod * undulator.nperiods * 1000.0
...
def motion2particles(aMotion, particle0):
    ...
    for i in range(npoints_traj):
        particle = Particle()
        particle.x = aMotion[i] / 1000.0
        particle.px = aMotion[3 * npoints_traj + i]
        particle.y = aMotion[npoints_traj + i] / 1000.0
        particle.py = aMotion[4 * npoints_traj + i]
        particle.s = particle0.s + aMotion[2 * npoints_traj + i] / 1000.0
...
def und_trace(undulator, particle0, energy, bRough, n_trajectory_points, accuracy=1):
    ...
    npoints_traj = n_trajectory_points * undulator.field_map.field_file_rep - (
        undulator.field_map.field_file_rep - 1
    ) * (2 - bRough)
    ...
    init_cond = [
        ...
        particle0.px,
        particle0.py,
        energy * (1 + particle0.p) / m_e_GeV,
    ]

    # print n_trajectory_points*11
    # print len(motion.memory_motion)
    error = cundul.trajectory(
        Py2C(
            magField
        ),  # 1D magnetic field array [X,Y,Z,Bx,By,Bz] or [Z,By] or [Z,By,Bx]
        colsMF,  # number of cols in magnetic data
        lenMF,  # number of rows in magnetic data
        Py2C(
            [
                undulator.dx,  # misaligmnent of undulator  [dx, dy, v_angle, h_angle, tilt]
                undulator.dy,
                0.0,
                0.0,
                undulator.dtilt,
        ...
        c_int(
            bRough
        ),  # if bRough = 1 then |  |  | else  | . . . (|) . . . | (gauss integration)
        Py2C(
            init_cond
        ),  # [beam.x [mm], beam.y [mm], 0 [long. pos. mm], beam.xp [rad!], beam.yp[rad!], gamma]
        Py2C(
            undul_param
        ),  # [Bx (amplitude [T]), By (amplitude [T]), phase (between Bx and By),
        # number_of_periods, lenPeriod [m], ax (width of undulator [m])]
        n_trajectory_points,  # number of points on trajectory on element
        c_int(
            fm.field_file_rep
        ),  # Number of repetition of field file. for example, undul = (end1, period*40, end2)
        aMotion.ctypes.data_as(dPtr),
    )  # pointer to array of motion
    ...
    for element in lattice.sequence:
        if element.type == "undulator":
            ...
            if energy == 0:
                ...
                list_particle = trajectory_through_undul(
                    element, particle, energy, bRough=0, n_trajectory_points=ndiv_und
                )
