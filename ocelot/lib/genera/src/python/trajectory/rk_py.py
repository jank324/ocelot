...
def track_und_RK(y0, h, N, kz, kx, Kx, energy):
    ...
    cmm = 299792458
    massElectron = 0.510998910e6  # // rest mass of electron

    u = np.zeros((N * 9, len(y0) / 6))
    px = y0[1::6]
    py = y0[3::6]
    dz = h
    dGamma2 = 1.0 - 0.5 / (gamma * gamma)
    pz = dGamma2 - (px * px + py * py) / 2.0
    ...
    for i in range(N - 1):
        ...
        kx2 = bx * dz
        ky2 = by * dz
        Bx, By, Bz = fields(X + kx1 / 2.0, Y + ky1 / 2.0, Z + dz / 2.0, kx, ky, kz, B0)
        ...
        kx3 = bx * dz
        ky3 = by * dz
        Bx, By, Bz = fields(X + kx2 / 2.0, Y + ky2 / 2.0, Z + dz / 2.0, kx, ky, kz, B0)
        ...
        # u[(i+1)*6 + 1] = betax
        # u[(i+1)*6 + 3] = betay
    u[(N - 1) * 9 + 6], u[(N - 1) * 9 + 7], u[(N - 1) * 9 + 8] = fields(
        u[(N - 1) * 9 + 0], u[(N - 1) * 9 + 2], u[(N - 1) * 9 + 4], kx, ky, kz, B0
    )
    return u
