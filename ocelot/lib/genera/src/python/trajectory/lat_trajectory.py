...
def integration_beta2(s, px, py):
    ...
    for i in xrange(0, len(px2) - 2, 2):
        Ix += (px2[i] + 4.0 * px2[i + 1] + px2[i + 2]) * h / 6.0
        Iy += (py2[i] + 4.0 * py2[i + 1] + py2[i + 2]) * h / 6.0
...
def x2x(X):
    sqrt35 = 0.5 * sqrt(3.0 / 5.0)

    xnew = [X[0]]
    h = X[1] - X[0]
    xgaus = (
        array([0.5 - sqrt35, 0.5 - sqrt35 + sqrt35, 0.5 - sqrt35 + sqrt35 + sqrt35]) * h
    )
    # print h, xgaus
    for x in X:
        for xg in xgaus:
