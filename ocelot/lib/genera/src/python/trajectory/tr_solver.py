...
from numpy import array, delete, split, where
from undulator import define_points_undul, field_map_for_dll
    ...
    for part in parts_cell:
        ...
        for elem in part:
            ...
            if elem.type == und_type:  # structure.type == "undulator"
                ...
                if elem.field_map:
                    size_motion = size_motion * elem.field_map.field_file_rep - (
                        elem.field_map.field_file_rep - 1
                    ) * (2 - bRough)
...
def c_format_cell(parts_cell):
    ...
    for part in parts_cell:
        ...
        for elem in part:
            ...
            if elem.type == "drift":
                line = [10, length, 0.0, 0.0, 0.0, npoints]
            elif elem.type == "sbend":
                line = [20, length, elem.k1 * 1e-6, elem.angle / length, 0.0, npoints]
            elif elem.type == "rbend":
                h = elem.angle / length  # 1/ro
                phi = elem.angle / 180.0 * pi
                line = [
                    25,
                    0.0,
                    phi / 2,
                    ...
                    0.0,
                    0,
                ]  # first edge. See trajectory.accelerator_physics
                structure.extend(line)
                line = [20, length, elem.k1 * 1e-6, h, 0.0, npoints]
                structure.extend(line)
                line = [25, 0.0, phi / 2, elem.angle / length, 0.0, 0]  # second edge
            elif elem.type == "quadrupole":
                line = [30, length, elem.k1 * 1e-6, 0.0, 0.0, npoints]
            elif elem.type == "undulator":
                line = [40, 0.0, 0.0, 0.0, 0.0, npoints]
...
def prepare_field(part_cell):
    ...
    for elem in part_cell:
        if elem.type == und_type:
            mag_field, len_mf, cols_mf, undul_param = field_map_for_dll(
                elem
            )  # params in [T] and [mm]
...
def trajectory_body(cell, beam, accuracy=1, mode_traj="radiation"):
    ...
    define_status(
        cell, beam, mode_traj
    )  # fast or classic calculation: only for ideal undulator with infinite width
    ...
    for elem in cell.sequence:
        ...
        # 2
    parts_cell = divide_cell(
        cell.sequence
    )  # it will divide CELL on parts, ones will include undulator or not.
    # 3
    ...
    for i, part in enumerate(parts_cell):
        ...
        # print c_magField
        ret = myfunc.trajectory2(
            c_magField,  # 1D magnetic field array [X,Y,Z,Bx,By,Bz] or [Z,By] or [Z,By,Bx]
            ...
            c_int(size),
        )  # number of elements = len(c_structure)/6
        errorsBlock(ret)
