...
class PerfDB:
    ...
    def new_tuning(self, params):
        ...
        with self.db:
            ...
            cursor.execute(
                "insert into TUNINGS(TIME,CHARGE,WL, COMMENT) VALUES(?,?,?,?)",
    ...
    def new_action(self, tuning_id, start_sase, end_sase):
        ...
        with self.db:
            cursor = self.db.cursor()
            cursor.execute(
                "insert into ACTIONS(TUNING_ID,SASE_START,SASE_END) VALUES(?,?,?)",
    ...
    def get_actions(self, tuning_id=None):
        cursor = self.db.cursor()
        if tuning_id == None:
            tuning_id = self.current_tuning_id()
    ...
    def add_action_parameters(
        self, tuning_id, action_id, param_names, start_vals, end_vals
    ):
    ...
    def get_action_parameters(self, tuning_id, action_id):
        cursor = self.db.cursor()
        cursor.execute(
            "select * from PARAMETERS WHERE TUNING_ID=:tid and ACTION_ID = :aid",
    ...
    def add_machine_parameters(self, tuning_id, params):
        ...
        with self.db:
            ...
            for k in params.keys():
                cursor.execute(
                    "insert into MACHINE_STATE(TUNING_ID,PAR_NAME,PAR_VALUE) VALUES(?,?,?)",
    ...
    def get_machine_parameters(self, tuning_id):
        cursor = self.db.cursor()
        cursor.execute(
            "select * from MACHINE_STATE WHERE TUNING_ID=:tid", {"tid": tuning_id}
        )
        return cursor.fetchall()
...
def test_new_tunings(dbname):
    ...
    db.new_tuning(
        {"wl": 13.6, "charge": 0.1, "comment": "test tuning"}
    )  # creates new tuning record (e.g. for each shift);
...
def test_new_action(dbname):
    ...
    print(
        "current actions in tuning",
        [(t.id, t.tuning_id, t.sase_start, t.sase_end) for t in db.get_actions()],
    )
...
def test_add_action_parameters(dbname):
    ...
    db.add_action_parameters(
        tune_id,
        action_id,
        param_names=["H1", "Q1", "test"],
        start_vals=[0.1, 0.2, "test"],
        end_vals=[1.1, 1.3, "gh"],
    )
    print(
        "current actions",
    ...
    print(
        ...
        [
            (p.tuning_id, p.action_id, p.par_name, p.start_value, p.end_value)
            for p in db.get_action_parameters(tune_id, action_id)
        ],
    )
