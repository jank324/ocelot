...
class LocalLauncher(Launcher):
    ...
    def launch(self):
        ...
        try:
            ...
            print(
                "os error:", err.errno, err.strerror, "initial cmd is:\n", self.program
            )
            if hasattr(self, "output"):
...
class MpiQueueLauncher(Launcher):
    ...
    def submit(self):
        ...
        _logger.info(
            "submitting job to MAXWELL cluster queue 1/2 (assembling cmd line)"
        )
        ...
        _logger.debug(
            "running command: {0:s} (remark/TODO: this does not print quotes around the arguments)".format(
                " ".join(args)
            )
        )
        ...
        try:
            ...
            _logger.info(
                "job submitted: jobid={0}, sbatch returned status {1}".format(
                    str(self.jobid), str(rc)
                )
            )
        except ValueError:
            ...
            self.jobid_valid = False
            _logger.info(
                "unclear if job submission successful, sbatch returned status {0}".format(
                    str(rc)
                )
            )
    ...
    def update_job_state(self):
        ...
        if self.jobid_valid == False:
            print(
                "error: update_job_state: jobid is not valid. Was job submission successful?"
            )
            return False
        ...
        p = subprocess.Popen(
            [
                ...
                "--noheader",
                "--format",
                "JobID,State,Partition,AllocCPUs,Elapsed",
                "-j",
                str(self.jobid),
            ],
            stdout=subprocess.PIPE,
        ...
        p.wait(
            timeout=10
        )  # exit code is only available after termination of sub-process
        ...
        if gotit:
            ...
            print(
                "unable to extract job status from 'sacct' output, exit code was: {0}".format(
                    rc
                )
            )
    ...
    def wait_for_completion(self, verbose=False):
        ...
        while True:
            ...
            if verbose:
                print(
                    "job {0}: state={1}, runtime={2}, is_complete={3}".format(
                        self.jobid, self.jobstate, self.jobtime, is_complete
                    )
                )
            if is_complete:
                break
...
class SshMpiLauncher(Launcher):
    ...
    def collect(self, inputDir, outputDir, pattern="*"):
        ...
        command1 = (
            ...
            + " 'cd "
            + self.dir
            + "; python merge.py simulation.gout.slice >> simulation.gout'"
        )
        command2 = (
            "scp -r "
            + self.port.replace("-p", "-P")
