...
def dfl_st_cpl(dfl, theta_b, inp_axis="y", s_start=None):
    ...
    if inp_axis == "y":
        for i in np.where(shift_m_scale != 0)[0]:
            fld[i, :, :] = np.roll(
                fld[i, :, :], -direction * shift_pix_scale[i], axis=0
            )
            # if direction==1:
        ...
        for i in np.where(shift_m_scale != 0)[0]:
            fld[i, :, :] = np.roll(
                fld[i, :, :], -direction * shift_pix_scale[i], axis=1
            )
            # if direction==1:
            # fld[i,:,:abs(shift_pix_scale[i])]=0
...
def dfl_hxrss_filt(
    ...
    idx = fwhm3(trf_m)[2]
    cwidth = abs(
        trf.k[idx[1]] - trf.k[idx[0]]
    )  # width the crystal transfer function hole (amplitude)
    ...
    if s_delay > dfl_z_mesh * padn:
        raise Exception(
            "s_delay %.2e is larger that the padded dfl %.2e Consider using enforce_padn > %.2f"
            % (s_delay, dfl_z_mesh * padn, s_delay / dfl_z_mesh)
        )
    ...
    if dump_proj:
        ...
        _logger.debug(
            ind_str
            + "done in {:.2f} sec, (inkl. {:.2f} sec for proj calc)".format(
                t_func, t_proj
            )
        )
        return (
            (t_l_scale, None, t_l_int_a, t_l_pha_a),
            (f_l_scale, f_l_filt, None, f_l_int_a),
        )  # f_l_int_b,t_l_int_b,
...
def save_xhrss_dump_proj(dump_proj, filePath):
    ...
    (t_l_scale, _, t_l_int_a, t_l_pha_a), (
        f_l_scale,
        f_l_filt,
        _,
        f_l_int_a,
    ...
    np.savetxt(
        f,
        np.c_[t_l_scale, t_l_int_a, t_l_pha_a],
        header=header,
        fmt="%e",
        newline="\n",
        comments="",
    ...
    np.savetxt(
        f,
        np.c_[f_l_scale, f_l_int_a, np.abs(f_l_filt), np.unwrap(np.angle(f_l_filt))],
        header=header,
        fmt="%.8e",
        newline="\n",
        comments="",
...
def create_fel_lattice(
    ...
    **kwargs
    ...
    qfh = Quadrupole(l=qf.l / 2.0)
    qdh = Quadrupole(l=qd.l / 2.0)
...
def create_fel_lattice_tmp(
    ...
    **kwargs
    ...
    qfh = Quadrupole(l=qf.l / 2.0, k1=1)
    qdh = Quadrupole(l=qd.l / 2.0, k1=-1)
...
def prepare_el_optics(beam, lat_pkg, E_photon=None, beta_av=None, s=None):
    ...
    if beta_av is None:
        ...
        raise_min_beta = True
        _logger.info(
            "optimal minimal average beta function was estimated and set to {:.3f}m".format(
                beta_av
            )
        )
    else:
...
def rematch(beta_mean, l_fodo, qdh, lat, extra_fodo, beam, qf, qd):
    ...
    k, betaMin, betaMax, __ = fodo_parameters(
        betaXmean=beta_mean, L=l_fodo, verbose=True
    )
...
def rematch_beam_lat(beam, lat_pkg, beta_mean, raise_min_beta=False):
    ...
    if beta_mean <= 2 * l_fodo:
        if raise_min_beta:
            _logger.warning(
                "Desired average beta function {}m was not larger that cell length and was set to that length {}m".format(
            ...
            _logger.error(
                "Desired average beta function {}m should be larger that cell length {}m".format(
                    beta_mean, 2 * l_fodo
                )
            )
            raise ValueError(
    ...
    k, betaMin, betaMax, __ = fodo_parameters(
        betaXmean=beta_mean, L=l_fodo, verbose=False
    )
...
def checkout_run(run_dir, run_id, prefix1, prefix2, save=False, debug=1):
    ...
    if save:
        ...
        os.system(
            "cp " + old_file + ".dfl " + new_file + ".dfl 2>/dev/null"
        )  # 2>/dev/null to supress error messages if no such file
        ...
        os.system(
            "mv " + old_file + ".dfl " + new_file + ".dfl 2>/dev/null"
        )  # 2>/dev/null to supress error messages if no such file
        if debug > 0:
...
def calc_ebeam_modulation_in_undulator(
    ...
    sig_s = (
        ...
        * speed_of_light
        * laser_t_fwhm
        / (2.35 * und_Nperiods * laser_wavelength)
    )
    ...
    dgam_dz = (
        (16 * np.pi * laser_E * und_Nperiods * ksi / laser_t_fwhm / P0) ** 0.5
        * JJ
        * (q / (1 + q**2 * z**2)) ** 0.5
        * np.cos(
            laser_phase_shift
            ...
            + 2 * np.pi * detuning * z
            + np.arctan(q * z)
            - 2 * np.pi * und_Nperiods * s
            - x0**2 * np.sin(2 * np.pi * z) ** 2 / (1 + q**2 * z**2)
        )
        * np.exp(-(x0**2) * np.sin(2 * np.pi * z) ** 2 / (1 + q**2 * z**2))
        * np.exp(-((z - s) ** 2) / (2 * sig_s**2))
    )
