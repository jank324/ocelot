...
from sim_info import RunInfo, SimInfo
...
class RunStatCanvas(MyMplCanvas):
    ...
    def update_figure(self):
        ...
        rects = self.axes.bar(
            self.data.run_ids, self.data.max_powers, width=0.2, color="b", alpha=0.5
        )
    ...
    def update_figure(self):
        try:
            ...
            for i in range(start_id, self.data.show_run_id + 1):
                ...
                si += np.roll(
                    np.abs(self.data.runs[i].spec) ** 2, len(r.freq_ev) / 2
                ) / int(self.data.show_run_id + 1 - start_id)

            x = np.roll(
                r.freq_ev, len(r.freq_ev) / 2
            )  # [ 4*len(r.spec)/10:6*len(r.spec)/10]
            y = np.roll(
                np.abs(r.spec) ** 2, len(r.freq_ev) / 2
            )  # [ 4*len(r.spec)/10:6*len(r.spec)/10]
            ...
            self.axes.legend(
                [p1],
                ["run " + str(self.data.show_run_id) + " " + str(spec_width) + " ev"],
            )
...
class ApplicationWindow(QtGui.QMainWindow):
    ...
    def update_data(self):
        ...
        for r_id in range(len(self.data.run_ids)):
            ...
            if (
                run_id > self.data.show_run_id
                and self.data.stages[r_id] == self.data.stage
            ):
                self.data.show_run_id = run_id
