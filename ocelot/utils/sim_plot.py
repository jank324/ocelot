...
for run_id in runs:
    ...
    if args.stage in ["1", "3", "5"]:
        ...
        if args.stage == "5":
            run_file = run_dir + "/run." + str(run_id) + ".gout"
        ...
        if args.field_file:
            ...
            for i in xrange(len(P)):
                P[i] = sum(
                    np.abs(np.multiply(slices[i, :, :], slices[i, :, :].conjugate()))
                )
        ...
        run_file = (
            run_dir + "/run." + str(run_id) + ".s" + str(int(args.stage) - 1) + ".gout"
        )
        field_file = "tmp" + str(args.stage) + ".dfl"
        print("reading", run_file, "and", field_file)
        g = readGenesisOutput(run_file)
        slices = readRadiationFile(
            fileName=run_dir + "/" + field_file, npoints=g("ncar")
        )
        P = np.zeros_like(slices[:, 0, 0])
    ...
    x2 = h * c / (x_ticks) * 1.0e9  # coordinates in nm
