...
def I3_ID(L, h0):
    return 4.0 * L / (3 * pi) * h0**3


def I4_ID(L, h0, lu):
    return -3.0 * L * lu * lu * h0**4 / (32 * pi)
...
def I5_ID(L, h0, lu, beta_xc, Dx0, Dxp0):
    ...
    I = (
        ...
        / (108000 * pi**5 * beta_xc)
        * (
            144000 * pi**4 * (Dx0**2 + beta_xc**2 * Dxp0**2)
            + 13500 * (-1) ** nn * h0 * pi**3 * Dx0 * lu**2
            + 15656 * h0**2 * lu**4
            ...
            * pi**2
            * lu**2
            * (480 * Dx0 + h0 * (4 * L**2 + 48 * beta_xc**2 - lu**2))
        )
    )
    return I
        ...
        for tm in elem.first_order_tms:
            ...
            if elem.__class__ in (SBend, RBend, Bend) and tm.tm_type == TMTypes.MAIN:
                ...
                for z in np.linspace(0, elem.l, num=n_points_element, endpoint=True):
                    ...
                    Z.append(z)
                    Hx = (
                        tws_z.gamma_x * tws_z.Dx * tws_z.Dx
                        + 2.0 * tws_z.alpha_x * tws_z.Dxp * tws_z.Dx
...
class EbeamParams:
    def __init__(self, lattice, tws0, coupling=0.01, nsuperperiod=1):
        ...
        self.tau0 = (
            2.0 * self.E * 1000.0 * self.Tperiod / self.U0 if self.U0 != 0.0 else np.nan
        )
        ...
        self.sigma_x = np.sqrt(
            (self.sigma_e * self.tws0.Dx) ** 2 + self.emitt_x * self.tws0.beta_x
        )
        self.sigma_y = np.sqrt(
            (self.sigma_e * self.tws0.Dy) ** 2 + self.emitt_y * self.tws0.beta_y
        )
        self.sigma_xp = np.sqrt(
            (self.sigma_e * self.tws0.Dxp) ** 2 + self.emitt_x * self.tws0.gamma_x
        )
        self.sigma_yp = np.sqrt(
            (self.sigma_e * self.tws0.Dyp) ** 2 + self.emitt_y * self.tws0.gamma_y
        )
    ...
    def integrals_id(self):
        ...
        for elem in self.lat.sequence:
            if elem.__class__ == Undulator:
                ...
                tws = trace_z(self.lat, self.tws0, [L, L + elem.l / 2.0])
                i2 = I2_ID(elem.l, h0)
                i3 = I3_ID(elem.l, h0)
                i4 = I4_ID(elem.l, h0, elem.lperiod)
                i5 = I5_ID(
                    elem.l, h0, elem.lperiod, tws[1].beta_x, tws[0].Dx, tws[0].Dxp
                )
                self.I2_IDs += i2
                self.I3_IDs += i3
                self.I4_IDs += i4
        ...
        self.emit_ID = (
            self.emittance
            * (1.0 + self.I5_IDs / self.I5)
            / (1 + (self.I2_IDs - self.I4_IDs) / (self.I2 - self.I4))
        )
        self.sigma_e_ID = self.sigma_e * np.sqrt(
            (1.0 + self.I3_IDs / self.I3)
            / (1 + (2 * self.I2_IDs + self.I4_IDs) / (2.0 * self.I2 + self.I4))
        )
        self.U0_ID = Cgamma * (self.E * 1000) ** 4.0 * self.I2_IDs / (2.0 * np.pi)
