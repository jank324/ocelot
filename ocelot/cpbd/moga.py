...
class Moga:
    ...
    def generate_ind(self):
        return [
            np.random.uniform(self.bounds_min[i], self.bounds_max[i])
    ...
    def init_deap_functions(self):
        ...
        self.toolbox.register(
            "individual", tools.initIterate, creator.Individual, self.generate_ind
        )
        self.toolbox.register(
    ...
    def eval_pop(self, pop):
        ...
        fitnesses = [
            self.toolbox.evaluate(
                x0=x, iter_data=(key, self.c_iter), args=self.fit_func_args
            )
            for key, x in zip(keys, data_in)
        ]
    ...
    def nsga2(self, fit_func, fit_func_args=[], init_pop=None, cxpb=0.95, mutpb=0.95):
        ...
        if self.log_print:
            print("Number of used CPU: %i" % MPI_SIZE)
