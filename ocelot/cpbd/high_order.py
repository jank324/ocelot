...
from ocelot.common.globals import m_e_eV, m_e_GeV, speed_of_light
...
try:
    ...
    _logger.debug(
        "high_order.py: module NUMBA is not installed. Install it to speed up calculation"
    )
    nb_flag = False
...
def t_nnn_transport(L, h, k1, k2, energy=0):
    ...
    L4 = L3 * L
    L5 = L4 * L
    dx = h / kx2 * (1.0 - cx) if kx != 0.0 else L * L * h / 2.0
    dx_h = (1.0 - cx) / kx2 if kx != 0.0 else L * L / 2.0

    # Integrals
    denom = kx2 - 4.0 * ky2
    I111 = 1.0 / 3.0 * (sx2 + dx_h)  #  I111 = Gx * cx**2
    I122 = dx_h * dx_h / 3.0  #  I122 = Gx * sx**2
    I112 = sx * dx_h / 3.0  #  I112 = Gx * cx*sx
    I11 = L * sx / 2.0  #  I11  = Gx * cx
    I10 = dx_h  #  I10  = Gx
    I33 = L * sy / 2.0  #  I33  = Gy * cy
    I34 = (sy - L * cy) / (2.0 * ky2) if ky != 0.0 else L3 / 6.0  #  I34  = Gy * sy
    I211 = sx / 3.0 * (1.0 + 2.0 * cx)
    I222 = 2.0 * dx_h * sx / 3.0
    I212 = 1.0 / 3.0 * (2 * sx2 - dx_h)
    I21 = 1.0 / 2.0 * (L * cx + sx)
    I22 = I11
    I20 = sx
    I43 = 0.5 * (L * cy + sy)
    ...
    if kx != 0:
        ...
        I16 = h / kx2 * (dx_h - L * sx / 2.0)  #  I16  = Gx * dx
        I166 = h2 / kx4 * (I10 - 2 * I11 + I111)  #  I166 = Gx * dx**2
        I216 = h / kx2 * (I21 - I211)
        I226 = h / kx2 * (I22 - I212)
        I26 = h / (2.0 * kx2) * (sx - L * cx)
        I266 = h2 / kx4 * (I20 - 2.0 * I21 + I211)

        I511 = h * (3.0 * L - 2.0 * sx - sx * cx) / (6.0 * kx2)
        I522 = h * (3.0 * L - 4 * sx + sx * cx) / (6.0 * kx4)
        I516 = h / kx2 * (I51 - I511)
        # I52  =  (dx - 4.*L*sx)/(2.*kx2)
        I52 = (2.0 * dx - h * L * sx) / (2.0 * kx2)
        I526 = h / kx2 * (I52 - I512)
        I50 = h * (L - sx) / kx2
        I566 = h2 / kx4 * (I50 - 2 * I51 + I511)
        I56 = (h2 * (L * (1.0 + cx) - 2.0 * sx)) / (2.0 * kx4)

    else:
        I116 = h * L4 / 24.0  #  I116 = Gx * cx*dx
        I12 = L3 / 6.0  #  I12  = Gx * sx
        I126 = h * L5 / 40.0  #  I126 = Gx * sx*dx
        I16 = h * L4 / 24.0  #  I16  = Gx * dx
        I166 = h2 * L5 * L / 120.0  #  I166 = Gx * dx**2
        I216 = h * L3 / 6.0
        I226 = h * L4 / 8.0
        I26 = h * L3 / 6.0
        I266 = h2 * L5 / 20.0
        ...
        I526 = h2 * L5 * L / 240.0
        I50 = h * L3 / 6.0
        I566 = h2 * h * L5 * L2 / 840.0
        I56 = h2 * L5 / 120.0

    if kx != 0 and ky != 0:
        I144 = (sy2 - 2.0 * dx_h) / denom  #  I144 = Gx * sy**2
        I133 = dx_h - ky2 * (sy2 - 2.0 * dx_h) / denom  #  I133 = Gx * cy**2
        I134 = (sy * cy - sx) / denom  #  I134 = Gx * cy*sy
        I313 = (kx2 * cy * dx_h - 2.0 * ky2 * sx * sy) / denom  #  I313 = Gy * cx*cy
        I324 = (2.0 * cy * dx_h - sx * sy) / denom  #  I324 = Gy * sx*sy
        I314 = (2.0 * cy * sx - (1.0 + cx) * sy) / denom  #  I314 = Gy * cx*sy
        I323 = (
            sy - cy * sx - 2.0 * ky2 * sy * dx_h
        ) / denom  #  I323 = Gy * sx*cy = (2*ky2/kx2*(1 + cx)*sy - cy*sx)/denom + sy/kx2
        # derivative of Integrals
        I244 = 2.0 * (cy * sy - sx) / denom
        I233 = sx - 2.0 * ky2 * (cy * sy - sx) / denom
        I234 = (kx2 * dx_h - 2.0 * ky2 * sy2) / denom
        I413 = ((kx2 - 2.0 * ky2) * cy * sx - ky2 * sy * (1.0 + cx)) / denom
        I424 = (cy * sx - cx * sy - 2.0 * ky2 * sy * dx_h) / denom
        I414 = ((kx2 - 2.0 * ky2) * sx * sy - (1.0 - cx) * cy) / denom
        I423 = (
            cy * dx_h * (kx2 - 2 * ky2) - ky2 * sx * sy
        ) / denom  #  I423 = I323' = ((2.*ky2)/kx2*(1 + cx)*cy - cx*cy - ky2*sx*sy)/denom + cy/kx2

    elif kx != 0 and ky == 0:
        I323 = (L - sx) / kx2
        I324 = 2.0 * (1.0 - cx) / kx4 - L * sx / kx2
        I314 = (2.0 * sx - L * (1.0 + cx)) / kx2
        I313 = (1.0 - cx) / kx2
        I144 = (-2.0 + kx2 * L2 + 2.0 * cx) / kx4
        I133 = (1.0 - cx) / kx2
        I134 = (L - sx) / kx2
        # derivative of Integrals
        I423 = (1.0 - cx) / kx2
        I424 = (sx - L * cx) / kx2
        # I414 = 2*cx/kx2 + sx
        I414 = (cx - 1.0) / kx2 + L * sx
        I413 = sx
        I244 = (2.0 * L - 2.0 * sx) / kx2
        I233 = sx
        I234 = (1.0 - cx) / kx2
    else:
        I144 = L4 / 12.0  #  I144 = Gx * sy**2
        I133 = L2 / 2.0  #  I133 = Gx * cy**2
        I134 = L3 / 6.0  #  I134 = Gx * cy*sy
        I313 = L2 / 2.0  #  I313 = Gy * cx*cy
        I324 = L4 / 12.0  #  I324 = Gy * sx*sy
        I314 = L3 / 6.0  #  I314 = Gy * cx*sy
        I323 = L3 / 6.0  #  I323 = Gy * sx*cy
        I244 = L3 / 3.0
        I233 = L
        I234 = L2 / 2.0
        I413 = L
    ...
    # print I323, L, kx, ky
    if kx == 0 and ky != 0:
        I336 = (h * L * (3.0 * L * cy + (2.0 * ky2 * L2 - 3.0) * sy)) / (24.0 * ky2)
        I346 = (h * ((3.0 - 2.0 * ky2 * L2) * L * cy + 3.0 * (ky2 * L2 - 1.0) * sy)) / (
            24.0 * ky4
        )
        I436 = I346
        I446 = (h * L * (-3.0 * L * cy + (3.0 + 2.0 * ky2 * L2) * sy)) / (24.0 * ky2)

        I533 = (h * (3.0 * L + 2.0 * ky2 * L3 - 3.0 * sy * cy)) / (24.0 * ky2)
        I534 = (h * (L2 - sy2)) / (8.0 * ky2)
        I544 = (h * (-3.0 * L + 2.0 * ky2 * L3 + 3.0 * sy * cy)) / (24.0 * ky4)
        ...
        I533 = (h * (denom * L - 2.0 * (denom + 2.0 * ky2) * sx + kx2 * cy * sy)) / (
            2.0 * denom * kx2
        )
    ...
    K2 = k2 / 2.0
    coef1 = 2.0 * ky2 * h - h3 - K2
    coef3 = 2.0 * h2 - ky2

    t111 = coef1 * I111 + h * kx4 * I122 / 2.0
    t112 = 2.0 * coef1 * I112 - h * kx2 * I112
    t116 = 2.0 * coef1 * I116 + coef3 * I11 - h2 * kx2 * I122
    t122 = coef1 * I122 + 0.5 * h * I111
    t126 = 2.0 * coef1 * I126 + coef3 * I12 + h2 * I112
    t166 = coef1 * I166 + coef3 * I16 + 0.5 * h3 * I122 - h * I10
    t133 = K2 * I133 - ky2 * h * I10 / 2.0
    t134 = 2.0 * K2 * I134
    t144 = K2 * I144 - h * I10 / 2.0

    t211 = coef1 * I211 + h * kx4 * I222 / 2.0
    t212 = 2.0 * coef1 * I212 - h * kx2 * I212
    t216 = 2.0 * coef1 * I216 + coef3 * I21 - h2 * kx2 * I222
    # print(I216, I21, I222, t216)
    t222 = coef1 * I222 + 0.5 * h * I211
    t226 = 2.0 * coef1 * I226 + coef3 * I22 + h2 * I212
    t266 = coef1 * I266 + coef3 * I26 + 0.5 * h3 * I222 - h * I20
    t233 = K2 * I233 - ky2 * h * I20 / 2.0
    t234 = 2.0 * K2 * I234
    t244 = K2 * I244 - h * I20 / 2.0
    ...
    t511 = coef1 * I511 + h * kx4 * I522 / 2.0
    t512 = 2.0 * coef1 * I512 - h * kx2 * I512
    t516 = 2.0 * coef1 * I516 + coef3 * I51 - h2 * kx2 * I522
    t522 = coef1 * I522 + 0.5 * h * I511
    t526 = 2.0 * coef1 * I526 + coef3 * I52 + h2 * I512
    t566 = coef1 * I566 + coef3 * I56 + 0.5 * h3 * I522 - h * I50
    t533 = K2 * I533 - ky2 * h * I50 / 2.0
    t534 = 2.0 * K2 * I534
    t544 = K2 * I544 - h * I50 / 2.0
    i566 = h2 * (L - sx * cx) / (4.0 * kx2) if kx != 0 else h2 * L3 / 6.0

    T511 = t511 + 1 / 4.0 * kx2 * (L - cx * sx)

    T512 = t512 - (1 / 2.0) * kx2 * sx2 + h * dx
    T516 = t516 + h * (sx * cx - L) / 2.0
    T522 = t522 + (L + sx * cx) / 4.0
    T526 = t526 + h * sx2 / 2.0
    T566 = t566 + i566
    T533 = t533 + 1 / 4.0 * ky2 * (L - sy * cy)
    T534 = t534 - 1 / 2.0 * ky2 * sy2
    T544 = t544 + (L + sy * cy) / 4.0
...
def t_nnn_mad(L, h, k1, k2, energy=0):
    ...
    L4 = L3 * L
    L5 = L4 * L
    dx = (1.0 - cx) / kx2 if kx != 0.0 else L * L / 2.0
    dx2 = dx * dx
    K1 = k1
    K2 = k2
    ...
    fx = (L - sx) / kx2 if kx2 != 0 else L3 / 6.0
    f2y = (L - s2y) / ky2 if ky2 != 0 else L3 / 6
    J1 = (L - sx) / kx2 if kx2 != 0 else L3 / 6.0
    J2 = (3.0 * L - 4.0 * sx + sx * cx) / (2 * kx4) if kx != 0 else L**5 / 20.0
    J3 = (
        (15 * L - 22.5 * sx + 9 * sx * cx - 1.5 * sx * cx * cx + kx2 * sx2 * sx)
        / (6 * kx4 * kx2)
        if kx != 0
        else L**7 / 56.0
    )
    ...
    T116 = (
        -h / 12 / beta * khk * (3 * sx * J1 - dx2)
        + 0.5 * h2 / beta * sx2
        + 0.25 / beta * K1 * L * sx
    )
    T126 = (
        -h / 12 / beta * khk * (sx * dx2 - 2 * cx * J2)
        + 0.25 * h2 / beta * (sx * dx + cx * J1)
        - 0.25 / beta * (sx + L * cx)
    )
    T166 = (
        -h2 / 6 / beta2 * khk * (dx2 * dx - 2 * sx * J2)
        + 0.5 * h3 / beta2 * sx * J1
        - 0.5 * h / beta2 * L * sx
        - 0.5 * h / (beta2) * igamma2 * dx
    )
    ...
    T216 = -h / 12 / beta * khk * (3 * cx * J1 + sx * dx) - 0.25 / beta * K1 * (
        sx - L * cx
    )
    T226 = -h / 12 / beta * khk * (3 * sx * J1 + dx2) + 0.25 / beta * K1 * L * sx
    T266 = (
        -h2 / 6 / beta2 * khk * (sx * dx2 - 2 * cx * J2)
        - 0.5 * h / beta2 * K1 * (cx * J1 - sx * dx)
    ...
    T336 = (
        0.5 * h / beta * K2 * (cy * Jd - 2 * K1 * sy * Jf)
        + 0.5 * h2 / beta * K1 * J1 * sy
        - 0.25 / beta * K1 * L * sy
    )
    T346 = (
        0.5 * h / beta * K2 * (sy * Jd - 2 * cy * Jf)
        + 0.5 * h2 / beta * J1 * cy
        - 0.25 / beta * (sy + L * cy)
    )
    ...
    T436 = (
        0.5 * h / beta * K1 * K2 * (2 * cy * Jf - sy * Jd)
        + 0.5 * h / beta * (K2 + h * K1) * J1 * cy
        + 0.25 / beta * K1 * (sy - L * cy)
    )
    T446 = (
        0.5 * h / beta * K2 * (2 * K1 * sy * Jf - cy * Jd)
        + 0.5 * h / beta * (K2 + h * K1) * J1 * sy
        - 0.25 / beta * K1 * L * sy
    )
    ...
    T516 = (
        h2 / 12 / beta2 * khk * (3 * dx * J1 - 4 * J2)
        + 0.25 * h / beta2 * K1 * J1 * (1 + cx)
        + 0.5 * h / beta2 * igamma2 * sx
    )

    T526 = (
        h2 / 12 / beta2 * khk * (dx * dx2 - 2 * sx * J2)
        + 0.25 * h / beta2 * K1 * sx * J1
        + 0.5 * h / beta2 * igamma2 * dx
    )
    T566 = (
        h3 / 6 / beta3 * khk * (3 * J3 - 2 * dx * J2)
        + h2 / 6 / beta3 * K1 * (sx * dx2 - J2 * (1 + 2 * cx))
        + 1.5 / beta3 * igamma2 * (h2 * J1 - L)
    )
    T533 = (
        -h / beta * K1 * K2 * Jf
        - 0.5 * h / beta * (K2 + h * K1) * J1
        + 0.25 / beta * K1 * (L - cy * sy)
    )
...
def fringe_ent(h, k1, e, h_pole=0.0, gap=0.0, fint=0.0):
    """
    ...
    tan_e = np.tan(e)
    tan_e2 = tan_e * tan_e
    phi = fint * h * gap * sec_e * (1.0 + np.sin(e) ** 2)
    R = np.eye(6)
    R[1, 0] = h * tan_e
    R[3, 2] = -h * np.tan(e - phi)

    T = np.zeros((6, 6, 6))
    T[0, 0, 0] = -h / 2.0 * tan_e2
    T[0, 2, 2] = h / 2.0 * sec_e2
    T[1, 0, 0] = h / 2.0 * h_pole * sec_e3 + k1 * tan_e
    T[1, 0, 1] = h * tan_e2
    T[1, 0, 5] = -h * tan_e
    T[1, 2, 2] = (
        -k1 + h * h / 2.0 + h * h * tan_e2
    ) * tan_e - h / 2.0 * h_pole * sec_e3
    T[1, 2, 3] = -h * tan_e2
    T[2, 0, 2] = h * tan_e2
...
def fringe_ext(h, k1, e, h_pole=0.0, gap=0.0, fint=0.0):
    """
    Calculate first and second order matrices for edge focusing element.
    Note: the first order (R) is shown here in the sake of completeness but it is duplicated
            in r_matrix.py module and used from there in optics.py
    ...
    T[0, 0, 0] = h / 2.0 * tan_e2
    T[0, 2, 2] = -h / 2.0 * sec_e2
    T[1, 0, 0] = h / 2.0 * h_pole * sec_e3 - (-k1 + h * h / 2.0 * tan_e2) * tan_e
    T[1, 0, 1] = -h * tan_e2
    T[1, 0, 5] = -h * tan_e
    T[1, 2, 2] = (-k1 - h * h / 2.0 * tan_e2) * tan_e - h / 2.0 * h_pole * sec_e3
    T[1, 2, 3] = h * tan_e2
    T[2, 0, 2] = -h * tan_e2
...
def H23(vec_x, h, k1, k2, beta=1.0, g_inv=0.0):
    """
    {x, px}, {y, py}, {sigma, ps}
    sigma = s - ct
    ps = (E - E0)/(p0*c)
    ...
    px1 = (
        -(h * h + k1) * x
        + h * ps_beta
        + (-h * (px2 + py2) - (2.0 * h * k1 + k2) * x2 + (h * k1 + k2) * y2) / 2.0
    )
    y1 = py * k
    ...
    sigma1 = (
        -(h * x) / beta
        - ((px2 + py2) / (2.0 * beta))
        - g_inv * g_inv / (beta * (1.0 + beta))
    )
    return [x1, px1, y1, py1, sigma1, 0.0]


def verlet(vec_x, step, h, k1, k2, beta=1.0, g_inv=0.0):
    """
    ...
    px2_py2 = px * px + py * py
    ps_beta = ps / beta
    x1 = (x + step * px * (1.0 - ps_beta)) / (1.0 - step * h * px)
    y1 = y + step * py * (1.0 + h * x1 - ps_beta)
    vec_x[4] = sigma + step * (
        -h * x1 / beta - px2_py2 / (2.0 * beta) - g_inv * g_inv / (beta * (1.0 + beta))
    )
    px_d = (
        -(h * h + k1 + (h * k1 + k2 / 2.0) * x1) * x1
        + h * ps_beta
        + (-h * px2_py2 + (h * k1 + k2) * y1 * y1) / 2.0
    )
...
def verlet1O(vec_x, step, h, k1, k2, beta=1.0, g_inv=0.0):
    """
    q_{n+1} = q_{n} + h * dH(p_{n}, q_{n+1})/dp
    p_{n+1} = p_{n} - h * dH(p_{n}, q_{n+1})/dq
    """
...
def sym_map(z, X, h, k1, k2, energy=0.0):
    if h != 0.0 or k1 != 0.0 or k2 != 0.0:
        step = 0.005
    else:
        step = z
    if step > z:
    ...
    if gamma != 0.0:
        g_inv = 1.0 / gamma
        beta = np.sqrt(1.0 - g_inv * g_inv)
    z_array = np.linspace(0.0, z, num=n)
    ...
    for z in range(len(z_array) - 1):
        ...
        px2_py2 = px * px + py * py
        x = (x + step * px * (1.0 - ps_beta)) / (1.0 - step * h * px)
        y = y + step * py * (1.0 + h * x - ps_beta)
        sigma = sigma + step * (-h * x / beta - px2_py2 / (2.0 * beta) - c4)

        px = px + step * (
            h * ps_beta + (-h * px2_py2 + c3 * y * y) / 2.0 - (c1 + c2 * x) * x
        )
...
def moments_py(bx, by, Bx, By, Bz, dzk):
    ...
    by2 = by * by
    bxy = bx * by
    sq = np.sqrt(1.0 + bx2 + by2)
    k = sq * dzk
    mx = k * (by * Bz - By * (1.0 + bx2) + bxy * Bx)
    my = -k * (bx * Bz - Bx * (1.0 + by2) + bxy * By)
    return mx, my
...
def rk_track_in_field(y0, s_stop, N, energy, mag_field, s_start=0.0):
    """
    ...
    dz = h
    beta0 = np.sqrt(1.0 - 1.0 / (gamma0 * gamma0))
    ...
    for i in range(N - 1):
        ...
        kx2 = bx * dz
        ky2 = by * dz
        Bx, By, Bz = mag_field(X + kx1 / 2.0, Y + ky1 / 2.0, Z + dz / 2.0)
        ...
        kx3 = bx * dz
        ky3 = by * dz
        Bx, By, Bz = mag_field(X + kx2 / 2.0, Y + ky2 / 2.0, Z + dz / 2.0)
        ...
        u[(i + 1) * 9 + 0] = X + 1 / 6.0 * (kx1 + 2.0 * (kx2 + kx3) + kx4)
        u[(i + 1) * 9 + 1] = bxconst + 1 / 6.0 * (
            mx1 + 2.0 * (mx2 + mx3) + mx4
        )  # // conversion in mrad
        u[(i + 1) * 9 + 2] = Y + 1 / 6.0 * (ky1 + 2.0 * (ky2 + ky3) + ky4)
        u[(i + 1) * 9 + 3] = byconst + 1 / 6.0 * (my1 + 2.0 * (my2 + my3) + my4)
        u[
            (i + 1) * 9 + 4
        ] = Z_n  # u[i*9 + 4] + dz*np.sqrt(1 + u[(i+1)*9 + 1]**2 + u[(i+1)*9 + 3]**2)
        u[(i + 1) * 9 + 5] = y0[5]
    ...
    u[(N - 1) * 9 + 6], u[(N - 1) * 9 + 7], u[(N - 1) * 9 + 8] = mag_field(
        u[(N - 1) * 9 + 0], u[(N - 1) * 9 + 2], u[(N - 1) * 9 + 4]
    )
    return u
...
def rk_field(rparticles, s_start, s_stop, N, energy, mag_field, long_dynamics=True):
    ...
    traj_data = rk_track_in_field(
        rparticles, s_stop, N, energy, mag_field, s_start=s_start
    )
    ...
    if long_dynamics:
        ...
        z_fin += (
            traj_data[4, :]
            + np.sum(dz * np.sqrt(1 + x1 * x1 + y1 * y1), axis=0)
            - ref_path
        )
...
def arcline(SREin, Delta_S, dS, R_vect):
    ...
    if True:
        R_vect_valid = np.all(np.isfinite(R_vect)) and not np.all(
            np.equal(R_vect, np.zeros((3, 1)))
        )
        if R_vect_valid:
            R = norm(R_vect)
            n_vect = R_vect / R
            ...
            if np.abs(np.dot(n_vect, e1)) > epsilon:
                R_vect_valid = False
                msg = (
                    "In arcline: invalid R_vect --> using Drift."
