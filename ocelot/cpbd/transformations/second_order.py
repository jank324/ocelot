...
from ocelot.cpbd.tm_utils import (
    SecondOrderMult,
    sym_matrix,
...
class SecondTM(Transformation):
    ...
    def from_element(
        ...
        return cls.create(
            entrance_tm_params_func=element.create_second_order_entrance_params
            if element.has_edge
            else None,
            delta_e_func=element.create_delta_e,
            main_tm_params_func=element.create_second_order_main_params,
            exit_tm_params_func=element.create_second_order_exit_params
            if element.has_edge
            else None,
            tm_type=tm_type,
            length=element.l,
    ...
    def t_apply(self, energy, X, U5666=0.0):
    ...
    def calculate_Tb(self, energy) -> np.ndarray:
        ...
        T_tilt = transfer_map_rotation(
            self.r_z_no_tilt(self.length, energy),
            self.t_mat_z_e(self.length, energy),
            self.tilt,
        )[1]
