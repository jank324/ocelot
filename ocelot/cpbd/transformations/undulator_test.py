...
class UndulatorTestTM(TransferMap):
    ...
    def map4undulator(self, u, z, lperiod, Kx, ax, energy, ndiv):
        kz = 2.0 * np.pi / lperiod
        if ax == 0:
            kx = 0
        else:
            kx = 2.0 * np.pi / ax
        zi = np.linspace(0.0, z, num=ndiv)
        h = zi[1] - zi[0]
        kx2 = kx * kx
        kz2 = kz * kz
        ky2 = kz * kz + kx * kx
    ...
    def map_function(self, X, energy: float):
        ...
        return self.map4undulator(
            X,
            self.delta_length if self.delta_length != None else self.length,
            params.lperiod,
            params.Kx,
            params.ax,
            energy,
