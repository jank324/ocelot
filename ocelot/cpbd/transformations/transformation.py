...
class Transformation(ABC):
    ...
    def __init__(
        ...
        self.length = (
            length if tm_type == TMTypes.MAIN else 0.0
        )  # entrance/exit functions (e.g Edges or Couplerkicks) do not have a length.
        self.delta_length = delta_length
        self._delta_e_func = (
            delta_e_func if tm_type == TMTypes.MAIN else None
        )  # entrance/exit functions (e.g Edges or Couplerkicks) do not change beam energy.
    ...
    def get_delta_e(self):
        if (
            self._delta_e_func and self.tm_type == TMTypes.MAIN
        ):  # Entrance and Exit tms (Edge TMs) don't have delta_e
            return self._delta_e_func(
                delta_length=self.delta_length, total_length=self.length
            )
        else:
            return 0.0
        ...
        **params,
        ...
        try:
            ...
            if not tm_params_func:
                raise NotImplementedError(
                    f"{'entrance' if tm_type == TMTypes.ENTRANCE else 'exit'} function is not set in {cls.__class__.__name__}'s __init__"
                )
        except AttributeError:
            raise NotImplementedError(
                f"The specific element have to implement the function {tm_params_func.__name__}."
            )
        if delta_length is not None and (delta_length > length):
            _logger.warning(
                "delta_l > length of element. Set delta_l == length of element."
            )
            delta_length = length
        return cls(
            create_tm_param_func=tm_params_func,
            delta_e_func=delta_e_func,
            tm_type=tm_type,
            length=length,
    ...
    def apply(self, prcl_series):
        ...
        if prcl_series.__class__ == ParticleArray:
            ...
            prcl_series.s += (
                self.delta_length if self.delta_length is not None else self.length
            )

        elif prcl_series.__class__ == Particle:
            p = prcl_series
            p.x, p.px, p.y, p.py, p.tau, p.p = self.map_function(
                np.array([[p.x], [p.px], [p.y], [p.py], [p.tau], [p.p]]), p.E
            )[:, 0]
            ...
            if False in (list_e[:] == list_e[0]):
                for p in prcl_series:
                    self.map(
                        np.array([[p.x], [p.px], [p.y], [p.py], [p.tau], [p.p]]),
                        energy=p.E,
                    )
                    ...
                    p.s += (
                        self.delta_length
                        if self.delta_length is not None
                        else self.length
                    )
            else:
                pa = ParticleArray()
            ...
            _logger.error(
                " TransferMap.apply(): Unknown type of Particle_series: "
                + str(prcl_series.__class__.__name)
            )
            raise Exception(
                " TransferMap.apply(): Unknown type of Particle_series: "
                + str(prcl_series.__class__.__name)
