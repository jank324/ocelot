...
class TransferMap(Transformation):
    ...
    def from_element(
        ...
        return cls.create(
            entrance_tm_params_func=element.create_first_order_entrance_params
            if element.has_edge
            else None,
    ...
    def mul_p_array(self, rparticles, energy=0.0):
        """
    ...
    def multiply_with_tm(self, tm: "TransferMap", length) -> "TransferMap":
        # TODO: check
        return TransferMap(
            create_tm_param_func=lambda energy: self.get_params(energy).multiply(
                tm.get_params(energy)
            ),
            length=length + tm.length,
            delta_e_func=self.get_delta_e() + tm.get_delta_e(),
    ...
    def __mul__(self, m):
        ...
        """
        try:
            return m.multiply_with_tm(
                self,
                self.delta_length if self.delta_length is not None else self.length,
            )
        except AttributeError as e:
            _logger.error(
                " TransferMap.__mul__: unknown object in transfer map multiplication: "
                + str(m.__class__.__name__)
            )
            raise Exception(
                " TransferMap.__mul__: unknown object in transfer map multiplication: "
                + str(m.__class__.__name__)
