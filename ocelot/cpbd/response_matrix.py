...
class MeasureResponseMatrix:
    ...
    def read_virtual_dispersion(self, E0):
        ...
        X1, Y1 = self.read_virtual_orbit(
            p_init=Particle(p=0.01, E=E0), write2bpms=False
        )
    ...
    def optical_func_params(self, tw_init=None):
        ...
        self.nu_x = tws[-1].mux / 2.0 / np.pi
        self.nu_y = tws[-1].muy / 2.0 / np.pi
...
class RingRM(MeasureResponseMatrix):
    ...
    def calculate(self, tw_init=None):
        ...
        for i, bpm in enumerate(self.bpms):
            kx = np.sqrt(bpm.beta_x) / (2.0 * sin_pnu_x)
            ky = np.sqrt(bpm.beta_y) / (2.0 * sin_pnu_y)
            for j, hcor in enumerate(self.hcors):
                mu_x = abs(bpm.phi_x - hcor.phi_x)
                h_resp[i, j] = (
                    kx * np.sqrt(hcor.beta_x) * np.cos(mu_x - np.pi * self.nu_x)
                )
            for n, vcor in enumerate(self.vcors):
                mu_y = abs(bpm.phi_y - vcor.phi_y)
                v_resp[i, n] = (
                    ky * np.sqrt(vcor.beta_y) * np.cos(mu_y - np.pi * self.nu_y)
                )
        m = len(self.bpms)
        kx = len(self.hcors)
        ky = len(self.vcors)
...
class LinacSimRM(MeasureResponseMatrix):
    ...
    def calculate(self, tw_init=None):
        ...
        for j, cor in enumerate(
            [item for sublist in [self.hcors, self.vcors] for item in sublist]
        ):
...
class ResponseMatrixJSON:
    ...
    def calculate(self, tw_init=None):
        ...
        if self.method != None:
            ...
            self.cor_names = np.append(
                [cor.id for cor in hcors], [cor.id for cor in vcors]
            )
            self.bpm_names = [bpm.id for bpm in bpms]
            self.matrix = self.method.calculate(tw_init=tw_init)
        else:
            print(
                "ResponseMatrix.method = None, Add the method, e.g. MeasureResponseMatrix"
            )
    ...
    def inject(self, cor_list, bpm_list, inj_matrix):
        ...
        if not np.array_equal(c_names, cor_list):
            logger.warning(
                " ResponseMatrix.inject: Origin response matrix has no correctors:"
            )
            print(cors2[np.in1d(cors2, c_names, invert=True)])
        if not np.array_equal(b_names, bpm_list):
            logger.warning(
                " ResponseMatrix.inject: Origin response matrix has no BPMs:"
            )
            print(bpm_list[b_i2[:]])
        ...
        for n in range(2):
            for i, c in enumerate(c_names):
                for j, b in enumerate(b_names):
                    ...
                    self.matrix[b_i1[j] + nb1 * n, c_i1[i]] = inj_matrix[
                        j + n * len(b_names), i
                    ]
                    # extr_matrix[j + n*len(b_names), i] = x1
    ...
    def dump(self, filename):
        ...
        dict_rmatrix["method_name"] = (
            self.method.__class__.__name__ if self.method != None else "None"
        )
    ...
    def show(self, list_cor=None, list_bpm=None):
        ...
        for i in range(np.shape(self.matrix)[1]):
            print(
                self.cor_names[i] + " " * (10 - len(self.cor_names[i])),
            )
...
class ResponseMatrix:
    ...
    def calculate(self, tw_init=None):
        ...
        if self.method != None:
            ...
            self.bpm_names = [bpm.id for bpm in bpms]
            self.matrix = self.method.calculate(tw_init=tw_init)
            self.df = self.data2df(
                matrix=self.matrix, bpm_names=self.bpm_names, cor_names=self.cor_names
            )
        else:
            print(
                "ResponseMatrix.method = None, Add the method, e.g. MeasureResponseMatrix"
    ...
    def retrieve_from_scan(self, df_scan):
        ...
        self.df = self.data2df(
            matrix=rm, bpm_names=self.bpm_names, cor_names=self.cor_names
        )
        return self.df
    ...
    def inject(self, cor_list, bpm_list, inj_matrix):
        ...
        :return:
        """
        df_slice = self.data2df(
            matrix=inj_matrix, bpm_names=bpm_list, cor_names=cor_list
        )
