...
def impedance2wake(f, y):
    ...
    df = f[1] - f[0]
    n = len(f)
    s = 1.0 / df * np.arange(n) / n * speed_of_light
    w = n * df * irfft(y, n)
    return s, w
...
def imp_resistiveAC_SI(f, cond, a, tau, L):
    ...
    for i in range(0, n):
        w = f[i] * f2w
        kw = cond / (1.0 + 1j * w * tau)
        Zs = np.sqrt(1j * w * mu_0 / kw) + 1j * w * L
        Z[i] = Zs / (f2w * a * (1.0 + w * Zs * koef))
    return Z
...
def pipe_wake(z, current, tube_radius, tube_len, conductivity, tau, roughness, d_oxid):
    ...
    eps_r = 2.0
    Ind = mu_0 * ((eps_r - 1.0) / eps_r * d_oxid + 0.01035 * roughness)
...
def xfel_pipe_wake(s, current):
    ...
    """
    conductivity = 3.66e7  # Ohm aluminium Dohlus
    tau = 7.1e-15  # s relaxation time - aluminium Dohlus
    tube_radius = 5e-3  # m radius
    tube_len = 1.0  # m length
    roughness = 600e-9
    d_oxid = 5e-9  # m thickness of oxide layer

    bunch, wake = pipe_wake(
        s, current, tube_radius, tube_len, conductivity, tau, roughness, d_oxid
    )
...
if __name__ == "__main__":
    ...
    conductivity = 3.66e7  # aluminium Dohlus
