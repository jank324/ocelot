...
class BendAtom(Magnet):
    ...
    def __str__(self):
        ...
        s += "angle=%8.6e, " % self.angle if self.angle != 0.0 else ""
        s += "e1=%8.6e, " % self.e1 if np.abs(self.e1) > 1e-15 else ""
        s += "e2=%8.6e, " % self.e2 if np.abs(self.e2) > 1e-15 else ""
        s += "tilt=%8.6e, " % self.tilt if self.tilt != 0.0 else ""
        s += "fint=%8.6e, " % self.fint if self.fint != 0.0 else ""
        s += "fintx=%8.6e, " % self.fintx if self.fint != 0.0 else ""
        s += "gap=%8.6e, " % self.gap if self.gap != 0.0 else ""
        s += "h_pole1=%8.6e, " % self.h_pole1 if self.h_pole1 != 0.0 else ""
        s += "h_pole2=%8.6e, " % self.h_pole2 if self.h_pole2 != 0.0 else ""
    ...
    def _R_edge(self, fint, edge):
        if self.l != 0.0:
            ...
            self.h = 0
        sec_e = 1.0 / np.cos(edge)
        phi = fint * self.h * self.gap * sec_e * (1.0 + np.sin(edge) ** 2)
        R = np.eye(6)
    ...
    def create_second_order_entrance_params(
        self, energy: float, delta_length: float = 0.0
    ) -> SecondOrderParams:
    ...
    def create_second_order_exit_params(
        self, energy: float, delta_length: float = 0.0
    ) -> SecondOrderParams:
