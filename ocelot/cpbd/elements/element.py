...
class Element:
    ...
    def __init__(self, eid=None, has_edge=False):
        ...
        self.l = 0.0
        self.tilt = 0.0  # rad, pi/4 to turn positive quad into negative skew
        self.dx = 0.0
        self.dy = 0.0
        self.params = {}
    ...
    def _default_B(self, R):
        return np.dot(
            (np.eye(6) - R),
            np.array([[self.dx], [0.0], [self.dy], [0.0], [0.0], [0.0]]),
        )
    ...
    def create_first_order_main_params(
        ...
        if delta_length is not None:
            R = uni_matrix(delta_length, 0.0, hx=hx, sum_tilts=0, energy=energy)
        else:
            R = uni_matrix(self.l, 0.0, hx=hx, sum_tilts=0, energy=energy)
        B = self._default_B(R)
