...
class Magnet(Element):
    def __init__(self, eid=None, has_edge=False):
        ...
        self.angle = 0.0  # Magnets Drift, Bend, Correctors (just angle)
        self.k1 = 0.0  # Magnets quadropole
        self.k2 = 0.0  # Magnets Sextupole
    ...
    def create_second_order_main_params(
        ...
        T = t_nnn(
            delta_length if delta_length is not None else self.l,
            0.0 if self.l == 0 else self.angle / self.l,
            self.k1,
            self.k2,
            energy,
        )
