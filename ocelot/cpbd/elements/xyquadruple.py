...
class XYQuadrupole(OpticElement):
    ...
    def __init__(
        self, l=0.0, x_offs=0.0, y_offs=0.0, k1=0.0, tilt=0.0, eid=None, tm=TransferMap
    ):
        if tm != TransferMap:
            tm = TransferMap
        super().__init__(
            XYQuadrupoleAtom(
                l=l, x_offs=x_offs, y_offs=y_offs, k1=k1, tilt=tilt, eid=eid
            ),
            tm=tm,
    ...
    def set_tm(self, tm: Transformation, **params):
        logger.debug(
            "Corrector Element only support CorrectorTM. Set tm to CorrectorTM."
        )
