...
def F(tm, B1, B2):
    ...
    for k in ks:
        t = np.tan(k) ** 2
        dI = (
            (2 * t + 1) ** 2 * (t / tm / (1 + t) - 1.0) / t
            + t
            - np.sqrt(t * tm * (1 + t))
            - (2 + 0.5 / t) * np.log(t / tm / (1 + t))
        )
...
def t_touschek(tws, beam):
    ...
    for i in range(len(tws) - 1):
        ...
        B1 = (
            betx**2
            / (2.0 * beta**2 * gam**2 * sig_x2)
            * (1.0 - sigh2 * Dxh**2 / sig_x2)
        )
        B1 += (
            bety**2
            / (2.0 * beta**2 * gam**2 * sig_y2)
            * (1.0 - sigh2 * Dyh**2 / sig_y2)
        )

        # B2 = B1**2 - tws[i].beta_x**2 * tws[i].beta_y**2 * sigh2 / (beta**4 * gam**4 * sig_x**4 * sig_y**4 * sig_e**2)* (sigp_x**2*sigp_y**2 - sig_e**4 * Dx**2 * Dy**2)

        B2 = (
            (1.0 - sigh2 * Dxh**2 / sig_x2) * betx**2 / sig_x2
            - (1.0 - sigh2 * Dyh**2 / sig_y2) * bety**2 / sig_y2
        ) ** 2 / (4 * beta**4 * gam**4)
        B2 += (sigh2**2 * betx**2 * bety**2 * Dxh**2 * Dyh**2) / (
            beta**4 * gam**4 * sig_x2 * sig_y2
        )
        ...
        I1 += (
            F(tm, B1, B2)
            / (np.sqrt(sig_x2 * sig_y2 - sig_e**4 * Dx**2 * Dy**2))
            * (tws[i + 1].s - tws[i].s)
        )

    return 1.0 / (
        rp2c * beam.Np * I1 / (8.0 * pi * gam**2 * beam.sigma_s) / tws[-1].s
    )
