...
try:
    ...
    logger.debug(
        "csr.py: module NUMBA is not installed." " Install it to speed up calculation"
    )
    nb_flag = False
...
try:
    ...
    logger.debug(
        "csr.py: module PYFFTW is not installed." " Install it to speed up calculation."
    )
...
try:
    ...
    logger.debug(
        "csr.py: module NUMEXPR is not installed." " Install it to speed up calculation"
    )
    ne_flag = False
    ...
    def Q2EQUI(self, q, BS_params, SBINB, NBIN):
        ...
        if IP_method == 1:
            for nb in range(N_BIN):
                ...
                a = ((3.0 * aa - bb) / 2.0) / step + 1.0
                k1 = min(Nz, max(1, np.floor(a)))
                b = ((aa + bb) / 2.0) / step + 1.0
                c = ((3.0 * bb - aa) / 2.0) / step + 1.0
...
class SubBinning:
    ...
    def subbin_bound(self, q, s, x_qbin, n_bin, m_bin):
        ...
        SBINB = np.interp(np.arange(K_BIN + 1.0) / K_BIN, aa, s)
        SBINB[0] = s[0]
        SBINB[K_BIN] = s[Ns - 1]
...
class K0_fin_anf:
    ...
    def K0_1_jit(indx, j, R, n, traj4, traj5, traj6, w, gamma):
        ...
        for i in range(indx - j):
            ...
            K[i] = (
                beta * (x - n0i * t4i - n1i * t5i - n2i * t6i)
                - b2 * (1.0 - t4 * t4i - t5 * t5i - t6 * t6i)
                - g2i
            ) * Ri_inv - (1.0 - beta * x) / w[i] * g2i
        return K
    ...
    def K0_fin_anf_opt(self, i, traj, wmin, gamma):
        ...
        R = np.zeros(i)
        w = np.zeros(i)
        j = self.K0_0(i, traj[0], traj[1], traj[2], traj[3], gamma, s, n, R, w, wmin)
    ...
    def K0_fin_anf_np(self, i, traj, wmin, gamma):
        ...
        K = (
            ...
            - b2 * (1.0 - t4 * t4_i - t5 * t5_i - t6 * t6_i)
            - g2i
        ) * R_inv - (1.0 - beta * x) / w * g2i
...
class CSR(PhysProc):
    ...
    def __init__(self, **kw):
        ...
        self.x_qbin = kw.get(
            "x_qbin", 0
        )  # length or charge binning; 0... 1 = length...charge
        self.n_bin = kw.get("n_bin", 100)  # number of bins
        self.m_bin = kw.get("m_bin", 5)  # multiple binning(with shifted bins)

        # smoothing
        self.ip_method = kw.get(
            "ip_method", 2
        )  # = 0 / 1 / 2 for rectangular / triangular / gauss
        self.sp = kw.get("sp", 0.5)  # ? parameter for gauss
        self.sigma_min = kw.get("sigma_min", 1.0e-4)  # minimal sigma, if ip_method == 2
        self.step_unit = kw.get(
            "step_unit", 0
        )  # if positive --> step=integer * step_unit

        # trajectory
        self.traj_step = kw.get("traj_step", 0.0002)  # [m] step of the trajectory
        self.energy = kw.get(
            "energy", None
        )  # [GeV], if None, beta = 1 and calculation of the trajectory with RK is not possible

        # CSR kick
        self.apply_step = kw.get(
            "apply_step", 0.0005
        )  # [m] step of the calculation CSR kick: csr_kick += csr(apply_step)
        self.step = kw.get(
            "step", 1
        )  # step in the unit steps, step_in_[m] = self.step * navigator.unit_step [m].
        # The CSR kick is applied at the end of the each step

        self.z_csr_start = kw.get(
            "z_csr_start", 0.0
        )  # z [m] position of the start_elem
        self.z0 = kw.get("z0", 0.0)  # self.z0 = navigator.z0 in track.track()

        self.end_poles = kw.get(
            "end_poles", False
        )  # if True magnetic field configuration 1/4, -3/4, 1, ...
        self.rk_traj = kw.get(
            "rk_traj", False
        )  # calculate trajectory of the reference particle with RK method
        ...
        self.sub_bin = SubBinning(
            x_qbin=self.x_qbin, n_bin=self.n_bin, m_bin=self.m_bin
        )
    ...
    def K0_fin_inf(self, i, traj, w_range, gamma):
        ...
        winf = s1 + winfms1
        s = winf + gamma * (
            gamma * (w_range - winf) - beta * np.sqrt(g2 * (w_range - winf) ** 2 + a2)
        )
        R = (w_range - s) / beta
    ...
    def K0_inf_inf(self, i, traj, w_range):
        ...
        if len(Nvalid) > 0:
            Nvalid = Nvalid[0]
            w = w_range[Nvalid:]
            s = (winf + w) / 2.0 + a2 / 2.0 / (winf - w)
            KS = (1.0 - np.dot(ev1, evo)) * np.log(
                0.5 * a2 / (w - winf) / (w - s)
            ) + np.dot(uup, evo) * (np.arctan((s - winf) / a) + np.pi / 2.0)
    ...
    def CSR_K1(self, i, traj, NdW, gamma=None):
        ...
        four_pi_eps0 = 1.0 / (1e-7 * speed_of_light**2)
    ...
    def prepare(self, lat):
        ...
        beta = (
            ...
            else np.sqrt(1.0 - 1.0 / (self.energy / m_e_GeV) ** 2)
        )
        self.csr_traj = np.transpose([[0, p.x, p.y, p.s, p.px, p.py, 1.0]])
        if (
            Undulator
            in [elem.__class__ for elem in lat.sequence[self.indx0 : self.indx1 + 1]]
            and not self.rk_traj
        ):
            self.rk_traj = True
            logger.warning(
                "CSR: Undulator element is in CSR section --> rk_traj = True"
            )
        for elem in lat.sequence[self.indx0 : self.indx1 + 1]:
            ...
            if elem.__class__ in [Bend, RBend, SBend] and not self.rk_traj:
                ...
                elem.__class__ in [Bend, RBend, SBend, XYQuadrupole, Undulator]
                ...
                if elem.__class__ is XYQuadrupole:
                    ...
                    By = elem.Kx * m_e_eV * 2.0 * pi / (elem.lperiod * speed_of_light)
                    Bx = elem.Ky * m_e_eV * 2.0 * pi / (elem.lperiod * speed_of_light)

                    def mag_field(x, y, z):
                        return (0, -By * np.cos(ku * z), 0)
                    ...
                    if self.end_poles:
                        ...
                        def mag_field(x, y, z):
                            return und_field(
                                x, y, z, elem.lperiod, elem.Kx, nperiods=elem.nperiods
                            )

                else:
                    delta_z = (
                        delta_s * np.sin(elem.angle) / elem.angle
                        if elem.angle != 0
                        else delta_s
                    )
                ...
                rparticle0 = np.array(
                    [
                        ...
                        [self.csr_traj[2, -1]],
                        [self.csr_traj[5, -1] / self.csr_traj[6, -1]],
                        [0],
                        [0],
                    ]
                )
                traj = rk_track_in_field(
                    rparticle0,
                    s_stop=delta_z,
                    N=N + 1,
                    energy=self.energy,
                ...
                xp2 = xp * xp
                yp2 = yp * yp
                zp = np.sqrt(1.0 / (1.0 + xp2 + yp2))

                s = cumtrapz(np.sqrt(1.0 + xp2 + yp2), z, initial=0)
        ...
        if self.pict_debug:
            ...
            ax3.plot(
                self.csr_traj[0, :], self.csr_traj[1 + 3, :] * 1000, "r", label=r"$X'$"
            )
            ax3.plot(
                self.csr_traj[0, :], self.csr_traj[2 + 3, :] * 1000, "b", label=r"$Y'$"
            )
            self.plt.legend()
    ...
    def apply(self, p_array, delta_s):
        ...
        SBINB, NBIN = self.sub_bin.subbin_bound(
            p_array.q_array, z[ind_z_sort], self.x_qbin, self.n_bin, self.m_bin
        )
        B_params = [
            self.x_qbin,
            self.n_bin,
        ...
        s1, s2, Ns, lam_ds = self.bin_smoth.Q2EQUI(
            p_array.q_array[ind_z_sort], B_params, SBINB, NBIN
        )
        st = (s2 - s1) / Ns
        sa = s1 + st / 2.0
        Ndw = [Ns - 1, st]
        ...
        h = max(1.0, self.apply_step / self.traj_step)
        ...
        z_sort = z[ind_z_sort]
        dE = np.interp(
            z_sort * (1.0 / st) + (0.0 - sa / st), np.arange(len(lam_K1)), lam_K1
        )
    ...
    def plot_wake(self, p_array, lam_K1, itr_ra, s1, st):
        ...
        ax1.plot(
            ...
            "r",
            self.csr_traj[0, itr_ra],
            self.csr_traj[1, itr_ra] * 1000,
            "bo",
        )
        ...
        ax2.plot(
            np.linspace(s1, s1 + st * len(lam_K1), len(lam_K1)) * 1000, lam_K1 * 1e-3
        )
...
class SaferCSR(CSR):
    ...
    def __init__(self, derbenev_criterion=0.5, sigma_min_factor=9.9, apply_kick=True):
        super().__init__()
    ...
    def query_derbenev_criterion(self, bunch_x, bunch_y, bunch_length):
        ...
        bending_plane_index = np.argmax(
            abs(tangent_post_step[:2] - tangent_pre_step[:2])
        )
