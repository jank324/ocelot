...
def show_output(g, show_field=False, output_file=None, show_slice=0):
    ...
    I = np.array(g.I)
    t = 1.0e15 * float(g("zsep")) * float(g("xlamds")) * np.arange(0, len(I)) / c
    f.add_subplot(133), plt.plot(t, power_s, lw=3), plt.plot(
        t, np.array(I) * np.max(power_s) / np.max(I), lw=3
    ), plt.grid(True)
    ...
    plt.figure()
    plt.plot(
        np.linspace(-dgrid / 2, dgrid / 2, npoints), abs(E[:, int(npoints / 2)]), lw=3
    ), plt.grid(True)
...
def plot_beam(fig, beam):
    ...
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), beam.I, "r", lw=3)
    plt.plot(1.0e6 * beam.z[beam.idx_max], beam.I[beam.idx_max], "bs")
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
    ax = ax.twinx()

    (p2,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e-3 * np.array(beam.eloss), "g", lw=3)
    ...
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), beam.g0, "r", lw=3)
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
    ax = ax.twinx()
    (p2,) = plt.plot(1.0e6 * np.array(beam.z), beam.dg, "g", lw=3)
    ...
    plt.grid(True)
    ax.set_xlabel(r"$\mu m$")
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), beam.ex, "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.z), beam.ey, "g", lw=3)
    plt.plot(1.0e6 * beam.z[beam.idx_max], beam.ex[beam.idx_max], "bs")
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
    ...
    plt.grid(True)
    ax.set_xlabel(r"$\mu m$")
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), beam.betax, "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.z), beam.betay, "g", lw=3)
    plt.plot(1.0e6 * beam.z[beam.idx_max], beam.betax[beam.idx_max], "bs")
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
    ...
    plt.grid(True)
    ax.set_xlabel(r"$\mu m$")
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.x), "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.y), "g", lw=3)
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
    ...
    plt.grid(True)
    ax.set_xlabel(r"$\mu m$")
    (p1,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.px), "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.py), "g", lw=3)
    ax.set_xlim([1.0e6 * beam.z[0], 1.0e6 * beam.z[-1]])
...
def plot_beam_2(fig, beam, iplot=0):
    ...
    if iplot == 0:
        ...
        (p2,) = plt.plot(
            1.0e6 * np.array(beam.z), 1.0e-3 * np.array(beam.eloss), "g", lw=3
        )
    ...
    if iplot == 2:
        ...
        (p1,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.ex), "r", lw=3)
        (p2,) = plt.plot(1.0e6 * np.array(beam.z), 1.0e6 * np.array(beam.ey), "g", lw=3)
