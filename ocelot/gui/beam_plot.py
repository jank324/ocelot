...
@if_plottable
def plot_beam(
    beam, figsize=3, showfig=True, savefig=False, fig=None, plot_xy=None, debug=0
):
    ...
    if plot_xy == None:
        if (
            np.mean(beam.x) == 0
            and np.mean(beam.y) == 0
            and np.mean(beam.xp) == 0
            and np.mean(beam.yp) == 0
        ):
            plot_xy = 0
        else:
            plot_xy = 1
    ...
    (p1,) = plt.plot(1.0e6 * np.array(beam.s), beam.I, "r", lw=3)
    plt.plot(1.0e6 * beam.s[idx], beam.I[idx], "bs")
    ...
    if hasattr(beam, "eloss"):
        if (beam.eloss != 0).any():
            ax_loss = ax_I.twinx()
            (p2,) = plt.plot(
                1.0e6 * np.array(beam.s), 1.0e-3 * np.array(beam.eloss), "g", lw=3
            )
            ax_loss.legend(
                [p1, p2], [r"$I [A]$", r"Wake $[KV/m]$"], fontsize=fontsize, loc="best"
            )
    ...
    ax_I.text(
        0.02,
        0.98,
        r"Q = {:.2f} pC".format(beam.charge() * 1e12),
    ...
    (p2,) = plt.plot(1.0e6 * np.array(beam.s), beam.dg, "g", lw=3)
    plt.plot(1.0e6 * beam.s[idx], beam.dg[idx], "bs")
    ax = ax.twinx()
    (p1,) = plt.plot(1.0e6 * np.array(beam.s), g_dev, "r", lw=3)
    plt.plot(1.0e6 * beam.s[idx], g_dev[idx], "bs")
    ax.legend([p1, p2], [r"$\gamma$ + " + str(g0), r"$\delta \gamma$"], loc="best")

    ax = fig.add_subplot(2 + plot_xy, 2, 3, sharex=ax)
    plt.grid(True)
    ax.set_xlabel(r"$s [\mu m]$", fontsize=fontsize)
    (p1,) = plt.plot(1.0e6 * np.array(beam.s), beam.emit_xn * 1e6, "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.s), beam.emit_yn * 1e6, "g", lw=3)
    plt.plot(1.0e6 * beam.s[idx], beam.emit_xn[idx] * 1e6, "bs")
    ax.set_ylim(ymin=0)

    ax.legend(
        [p1, p2],
        [r"$\varepsilon_x [\mu m]$", r"$\varepsilon_y [\mu m]$"],
        fontsize=fontsize,
        loc="best",
    )
    ...
    plt.grid(True)
    ax.set_xlabel(r"$s [\mu m]$", fontsize=fontsize)
    (p1,) = plt.plot(1.0e6 * np.array(beam.s), beam.beta_x, "r", lw=3)
    (p2,) = plt.plot(1.0e6 * np.array(beam.s), beam.beta_y, "g", lw=3)
    plt.plot(1.0e6 * beam.s[idx], beam.beta_x[idx], "bs")
    ax.set_ylim(ymin=0)
    ax.legend(
        [p1, p2], [r"$\beta_x [m]$", r"$\beta_y [m]$"], fontsize=fontsize, loc="best"
    )
    ...
    if plot_xy:
        ...
        plt.grid(True)
        ax.set_xlabel(r"$s [\mu m]$", fontsize=fontsize)
        (p1,) = plt.plot(1.0e6 * np.array(beam.s), 1.0e6 * np.array(beam.x), "r", lw=3)
        (p2,) = plt.plot(1.0e6 * np.array(beam.s), 1.0e6 * np.array(beam.y), "g", lw=3)

        ax.legend(
            [p1, p2], [r"$x [\mu m]$", r"$y [\mu m]$"], fontsize=fontsize, loc="best"
        )
        ...
        plt.grid(True)
        ax.set_xlabel(r"$s [\mu m]$", fontsize=fontsize)
        (p1,) = plt.plot(1.0e6 * np.array(beam.s), 1.0e6 * np.array(beam.xp), "r", lw=3)
        (p2,) = plt.plot(1.0e6 * np.array(beam.s), 1.0e6 * np.array(beam.yp), "g", lw=3)

        ax.legend(
            [p1, p2],
            [r"$x_p [\mu rad]$", r"$y_p [\mu rad]$"],
            fontsize=fontsize,
            loc="best",
        )

    ax.set_xlim([1.0e6 * np.amin(beam.s), 1e6 * np.amax(beam.s)])
...
def plot_beam_form_factor(
    form_factor: BeamFormFactor,
    x_axis_units="THz",
    x_axis_limits=(None, None),
    y_axis_limits=(None, None),
    figsize=(2 * 3, 3 * 3),
    normalize=True,
    fig_name="Beam Form Factor",
    log_scale=False,
    showtext=True,
    savefig=False,
    showfig=True,
    **kwargs
    ...
    if showtext:
        ax_current_profile.text(
            0.97,
            0.97,
            "Q={:.2f}pC".format(form_factor.beam_array.charge() * 1e12),
            horizontalalignment="right",
            verticalalignment="top",
            transform=ax_current_profile.transAxes,
            fontsize=12,
            color="black",
        )
    ...
    if x_axis_units in ["MHz", "mhz"]:
        ...
        _logger.warning(
            ind_str + "x_axis_units must one of 'MHz', 'GHz', 'THz', 'keV', 'eV', 'meV'"
        )
...
def plot_estimator_power_evo(fel, fig=None, und_duty_factor=1, **kwargs):
    ...
    plt.title(
        "Energy@{:.2f}m = {:.2e} J".format(fel.z_sat_min / und_duty_factor, E[-1])
    )
