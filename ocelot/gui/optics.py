...
from numpy import array, cos, log, pi, random, sign, sin, sqrt
...
def plot_geometry(ax, geo, proj="y"):
    ...
    for o in geo():
        ...
        if o.__class__ == Aperture:
            (li,) = ax.plot(
                [o.r[2], o.r[2]],
                [o.r[idx] + o.d[idx], o.r[idx] + o.size[idx]],
                color="#000000",
                lw=3,
            )
            (li,) = ax.plot(
        ...
        if o.__class__ == Crystal:
            (li,) = ax.plot(
                [o.r[2], o.r[2]],
                [o.r[idx] - o.size[idx], o.r[idx] + o.size[idx]],
                color="#999999",
                lw=3,
            )
...
def plot_rays(ax, rays, proj="x", alpha=0.4):
    for r in rays:
        ...
        for i in range(len(r.r0)):
            ...
            if proj == "x":
                ax.plot(
                    [r.r0[i][2], r.r0[i][2] + r.k[i][2] * r.s[i]],
                    [r.r0[i][0], r.r0[i][0] + r.k[i][0] * r.s[i]],
                    color="#006600",
                    lw=1,
                    alpha=alpha,
                )
            if proj == "y":
                ax.plot(
                    [r.r0[i][2], r.r0[i][2] + r.k[i][2] * r.s[i]],
                    [r.r0[i][1], r.r0[i][1] + r.k[i][1] * r.s[i]],
                    color="#006600",
                    lw=1,
                    alpha=alpha,
                )
