...
area.addDock(
    dock1, "left"
)  ## place dock1 at left edge of dock area (it will fill the whole space since there are no other docks yet)
        ...
        "children": [
            {
                "name": "Lattices",
                "type": "list",
                "values": lattice_names,
                "value": lattice_names[0],
            },
...
for v in sorted(globals().keys()):
    try:
        if globals()[v].__class__ == Quadrupole:
            params[0]["children"].append(
                {"name": v, "type": "float", "value": globals()[v].k1, "step": 0.1}
            )
        if globals()[v].__class__ == Drift:
            params[1]["children"].append(
                {"name": v, "type": "float", "value": globals()[v].l, "step": 0.1}
            )
        if globals()[v].__class__ in (SBend, RBend, Bend):
            params[2]["children"].append(
                {"name": v, "type": "float", "value": globals()[v].angle, "step": 0.1}
            )
    except:
        pass

if __tws__ != None:
    params[4]["children"].append(
        {"name": "beta_x", "type": "float", "value": __tws__.beta_x, "step": 0.1}
    )
    params[4]["children"].append(
        {"name": "alpha_x", "type": "float", "value": __tws__.alpha_x, "step": 0.1}
    )
    params[4]["children"].append(
        {"name": "beta_y", "type": "float", "value": __tws__.beta_y, "step": 0.1}
    )
    params[4]["children"].append(
        {"name": "alpha_y", "type": "float", "value": __tws__.alpha_y, "step": 0.1}
    )

if __knobs__ != None:
    for k in __knobs__.keys():
        params[5]["children"].append(
            {"name": k, "type": "float", "value": 0.0, "step": 0.1}
        )
...
w6 = pg.PlotWidget(title="\u03b2 x,y")  # beta
w6.plot([t.s for t in tws], [t.beta_x for t in tws], pen=(255, 0, 0))
w6.plot([t.s for t in tws], [t.beta_y for t in tws], pen=(0, 255, 0))
