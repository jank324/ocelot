...
def plot_dfl(
    ...
    **kwargs
    ...
    if dfl.Nx() == 1 or dfl.Ny() == 1:
        _logger.warning(
            ind_str
            + "plot_dfl() works only with RadiationFields, with dfl.Nx(), dfl.Ny() > 1"
        )
    ...
    _logger.debug(
        ind_str + "Lx = {:.2e}, Ly = {:.2e}, Lz = {:.2e}".format(leng_x, leng_y, leng_z)
    )
    ...
    if fig_name is None:
        if dfl_copy.fileName() == "":
            ...
            fig = plt.figure(
                "Radiation distribution" + suffix + " " + dfl_copy.fileName()
            )
    else:
    ...
    if log_scale:
        intplt = ax_int.pcolormesh(
            x,
            y,
            xy_proj,
            norm=colors.LogNorm(vmin=np.nanmin(xy_proj), vmax=np.nanmax(xy_proj)),
            cmap=cmap,
        )
    else:
    ...
    if phase == True:
        ax_ph = fig.add_subplot(
            2, 2 + column_3d, 4 + column_3d, sharex=ax_int, sharey=ax_int
        )
        ...
        ax_z.text(
            0.05,
            0.95,
            "fwhm={:.3g} ".format(fwhm_z) + r" [{:}]".format(unit_z),
            fontsize=12,
            horizontalalignment="left",
    ...
    if sum(x_line) != 0:
        try:
            x_line_f, rms_x = gauss_fit(
                x, x_line
            )  # fit with Gaussian, and return fitted function and rms
    ...
    if kwargs.get("showtext", True):
        try:
            ax_proj_x.text(
                0.95,
                0.95,
                "fwhm={:.3g} ".format(fwhm_x)
                + r" [{:}]".format(unit_xy)
                + "\nrms={:.3g}".format(rms_x)
                + r" [{:}]".format(unit_xy),
                horizontalalignment="right",
                verticalalignment="top",
                transform=ax_proj_x.transAxes,
                fontsize=12,
            )
        except:
            pass
    ...
    if kwargs.get("showtext", True):
        try:
            ax_proj_y.text(
                0.95,
                0.95,
                "fwhm={:.3g} ".format(fwhm_y)
                + r" [{:}]".format(unit_xy)
                + "\nrms={:.3g}".format(rms_y)
                + r" [{:}]".format(unit_xy),
                horizontalalignment="right",
                verticalalignment="top",
                transform=ax_proj_y.transAxes,
                fontsize=12,
            )
        except:
            pass
    ...
    if column_3d:
        ...
        if log_scale:
            ax_proj_xz.pcolormesh(
                z,
                x,
                np.swapaxes(xz_proj, 1, 0),
                norm=colors.LogNorm(vmin=min_xz_proj, vmax=np.nanmax(xz_proj)),
                cmap=cmap,
            )
        else:
        ...
        ax_proj_yz = fig.add_subplot(
            2, 2 + column_3d, 3, sharey=ax_int, sharex=ax_proj_xz
        )
        if log_scale:
            ax_proj_yz.pcolormesh(
                z,
                y,
                np.swapaxes(yz_proj, 1, 0),
                norm=colors.LogNorm(vmin=min_yz_proj, vmax=np.nanmax(yz_proj)),
                cmap=cmap,
            )
        else:
...
def plot_two_dfls(
    ...
    if None not in [label_first, label_second]:
        ax_y2.legend(
            fontsize=12, bbox_to_anchor=(0, 0.92), loc="upper left"
        )  # , loc=1)
        ax_y.legend(
            fontsize=12, bbox_to_anchor=(0, 0.995), loc="upper left"
        )  # , loc=2)
        ax_x2.legend(
            fontsize=12, bbox_to_anchor=(0, 0.92), loc="upper left"
        )  # , loc=1)
    ...
    if phase == True:
        ...
        ax_xy1.pcolormesh(
            dfl1.scale_x() * 1e6, dfl1.scale_y() * 1e6, np.sum(dfl1.intensity(), axis=0)
        )
        ax_xy2.pcolormesh(
            dfl2.scale_x() * 1e6, dfl2.scale_y() * 1e6, np.sum(dfl2.intensity(), axis=0)
        )
    ...
    _logger.info(
        ind_str
        + "plotting two dfls done in {:.2f} seconds".format(time.time() - start_time)
    )


def plot_wigner(*args, **kwargs):
    _logger.warning(
        "plotting wigner for time-frequency domain migrated to plot_wigner_z"
    )
...
def plot_wigner_z(
    ...
    **kwargs
    ...
    if log_scale != 0:
        if log_scale == 1:
            log_scale = 0.01
        wigplot = axScatter.pcolormesh(
            power_scale[::downsample],
            spec_scale[::downsample],
            wigner[::downsample, ::downsample],
            cmap=cmap,
            norm=colors.SymLogNorm(
                linthresh=v_maxabs * log_scale, linscale=2, vmin=v_min, vmax=v_max
            ),
            vmax=v_max,
            vmin=v_min,
        )
    ...
    if plot_text:
        ...
        axScatter.text(
            0.02,
            0.98,
            r"$W_{{max}}$= {:.2e}".format(np.amax(wigner)),
            horizontalalignment="left",
            verticalalignment="top",
            transform=axScatter.transAxes,
        )  # fontsize=12,
        if hasattr(W, "on_axis"):
            if W.on_axis == True:
                ...
                axScatter.text(
                    0.5,
                    0.98,
                    r"(assuming full spatial coherence)",
                    fontsize=10,
                    horizontalalignment="center",
                    verticalalignment="top",
                    transform=axScatter.transAxes,
    ...
    if plot_moments:
        ...
        plt.scatter(
            power_scale[idx_power_fine],
            inst_freq[idx_power_fine],
            s=weight_power[idx_power_fine],
            c="black",
            linewidths=2,
        )
        plt.scatter(
            group_delay[idx_spec_fine],
            spec_scale[idx_spec_fine],
            s=weight_spec[idx_spec_fine],
            c="green",
            linewidths=2,
        )
    ...
    if plot_proj:
        ...
        if plot_text:
            axHistx.text(
                0.02,
                0.95,
                r"E= {:.2e} J".format(W.energy()),
                horizontalalignment="left",
                verticalalignment="top",
                transform=axHistx.transAxes,
            )  # fontsize=12,
...
def plot_wigner_u(
    ...
    **kwargs
    ...
    # if W.domain in ['x','y','s']:
    if isinstance(W, WignerDistributionTransverse):
        power_scale, power_prefix, power_order = mprefix(
            W.s, order_bias=0.05, apply=add_prefix
        )
        spec_scale, spec_prefix, spec_order = mprefix(
            W.theta(), order_bias=0.05, apply=add_prefix
        )
        ...
        if x_units in ["s", "fs"]:
            power_scale, power_prefix, power_order = mprefix(
                -W.s / speed_of_light, order_bias=0.05, apply=add_prefix
            )
            # power_scale = -W.s / speed_of_light * 1e15
        ...
        if y_units in ["ev", "eV"]:
            spec_scale, spec_prefix, spec_order = mprefix(
                W.phen, order_bias=0.05, apply=add_prefix
            )
            # spec_scale = W.phen
            ...
            spec_scale, spec_prefix, spec_order = mprefix(
                W.freq_lamd, order_bias=0.05, apply=add_prefix
            )
            # spec_scale = W.freq_lamd
...
def plot_dfl_waistscan(sc_res, fig_name=None, figsize=4, showfig=True, savefig=False):
    ...
    if savefig != False:
        ...
        _logger.debug(
            ...
            + "to "
            + sc_res.filePath
            + "_{:.2f}m-{:.2f}m-waistscan.".format(sc_res.z_pos[0], sc_res.z_pos[-1])
            + str(savefig)
        )
        fig.savefig(
            sc_res.filePath
...
def plot_trf(trf, mode="tr", autoscale=0, showfig=True, savefig=None, fig_name=None):
    ...
    if autoscale:
        trf_s_fd_xlim = np.array(
            [trf.mid_k - n_width * trf.dk, trf.mid_k + n_width * trf.dk]
        )
    ...
    if hasattr(trf, "cryst"):
        title = (
            trf.cryst.lattice.element_name + " " + str(trf.cryst.ref_idx) + " " + mode
        )
    else:
        title = ""
...
def plot_stokes_values(
    ...
    **kwargs
    ...
    if np.size(sc) <= 1:
        _logger.warning(
            "plot_stokes_values needs more than a single point to plot (np.size(sc) <= 1)"
        )
        return
    ...
    if d_pol == "lin":
        plt.legend(
            ["$D_{lin}$", "$S_1$", "$S_2$", "$S_3$", "$S_0$"],
            loc="lower center",
            ncol=5,
            mode="expand",
            borderaxespad=0.5,
            frameon=1,
        ).get_frame().set_alpha(0.4)
    elif d_pol == 1:
        plt.legend(
            ["$D_{pol}$", "$S_1$", "$S_2$", "$S_3$", "$S_0$"],
            loc="lower center",
            ncol=5,
            mode="expand",
            borderaxespad=0.5,
...
def plot_stokes_angles(S, fig=None, showfig=True, direction="z", plot_func="scatter"):
    ...
    if np.size(sc) <= 1:
        _logger.warning(
            "plot_stokes_angles needs more than a single point to plot (np.size(sc) <= 1)"
        )
        return
...
def plot_stokes_3d(
    ...
    cmap_circ="bwr",
    figsize=4,
    fig_name="Visualization Stokes parameters",
    normalization="s0_max",
    cbars=True,
    savefig=False,
    showfig=True,
    text_present=True,
    **kwargs
    ...
    linear_plt = plot_stokes_sbfg_lin(
        ax1,
        stk_params,
        slice=z_plane,
        plane="z",
    ...
    plot_stokes_sbfg_lin(
        ...
        plane="x",
        cmap2d=cmap_lin,
        plot_title="Linear polarization",
        x_label="z",
        y_label="y",
        text_present=text_present,
        interpolation=interpolation,
    ...
    circular_plt = plot_stokes_sbfg_circ(
        ax4,
        stk_params,
        slice=z_plane,
        plane="z",
    ...
    plot_stokes_sbfg_circ(
        ...
        plane="x",
        cmap=cmap_circ,
        plot_title="Circular polarization",
        x_label="z",
        y_label="y",
        text_present=text_present,
        interpolation=interpolation,
    ...
    if cbars:
        ...
        if cbax1_dir == 1:
            ...
            plt.yticks(
                np.linspace(np.pi / 2, -np.pi / 2, 3), ["$-\pi/2$", "0", "$\pi/2$"]
        ...
        cbax2 = fig.add_axes(
            [0.91, 0.11, 0.04, 0.321]
        )  # This is the position for the colorbar [x, y, width, height]
...
def plot_stokes_sbfg_lin(
    ...
    **kwargs
    ...
    z_max, y_max, x_max = np.unravel_index(
        stk_params.s0.argmax(), stk_params.s0.shape
    )  # getting max element position

    if plane in ["x", 2]:
        swap_axes = True
        extent = [
            stk_params.sc_z[0] * 1e6,
            stk_params.sc_z[-1] * 1e6,
            stk_params.sc_y[0] * 1e6,
    ...
    m, n = psi_plane.shape
    linear_plt = imshow2d(
        np.array([psi_plane, lin_pol_plane]),
        ax=ax,
        cmap2d=cmap2d,
        extent=extent,
        interpolation=interpolation,
    ...
    if text_present:
        ...
        ax.text(
            0.97,
            0.97,
            dic.get(slice, "slice at {:}={:.3f} $\mu$m (max int)").format(
                plane, slice_pos * 1e6
            ),
            horizontalalignment="right",
            verticalalignment="top",
...
def plot_stokes_sbfg_circ(
    ...
    **kwargs
    ...
    if text_present:
        dic = {
            "proj": "projection",
            "max_slice": "slice at {:}={:.3f} $\mu$m (max int)",
        }
        ax.text(
            0.97,
            0.97,
            dic.get(slice, "slice at {:}={:.3f} $\mu$m").format(plane, slice_pos * 1e6),
            horizontalalignment="right",
            verticalalignment="top",
            transform=ax.transAxes,
            fontsize=10,
...
def plot_1d_hprofile(
    height_profile,
    figsize=4,
    fig_name="Height profile",
    savefig=False,
    showfig=True,
    **kwargs
):
    ...
    ax2.text(
        0.97,
        0.97,
        "surface height errors RMS = {0:.02e} [m]".format(height_profile.hrms()),
        horizontalalignment="right",
        verticalalignment="top",
