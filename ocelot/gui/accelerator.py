...
def plot_lattice(
    lat, axis, alpha=1.0, params={"kmax": 2.0, "ang_max": 0.5e-2}, s_start=0.0
):
    axis.grid(True)
    ...
    for e in rendered_seq:
        ...
        if e.__class__ in [Bend, SBend, RBend, Hcor, Vcor]:
            axis.add_patch(
                mpatches.Rectangle(
                    offs + np.array([pos, 0.0]),
                    dx,
                    np.sign(-e.angle) * min_dipole_height
                    - e.angle / ang_max * (1 - min_dipole_height),
                    color="#0099FF",
                    alpha=alpha,
                )
            )
        ...
        if e.__class__ in [Solenoid]:
            axis.add_patch(
                mpatches.Rectangle(
                    offs + np.array([pos, 0.0]),
                    dx,
                    np.sign(-e.k) * min_solenoid_height
                    - e.k / sol_max * (1 - min_solenoid_height),
                    color="#FF99FF",
                    alpha=alpha,
                )
            )
            ...
            if e.k1 >= 0:
                axis.add_patch(
                    mpatches.Ellipse(
                        offs + np.array([pos, 0.0]),
                        dx,
                        min_quad_height + abs(e.k1 / kmax) * 2,
                        color="green",
                        alpha=alpha,
                    )
                )
        ...
        if e.__class__ in [Undulator]:
            ...
            height = 0.7
            gap = 0.1
            if e.Kx**2 + e.Ky**2 < 1.0e-6:
                gap = 0.2

            for iseg in np.arange(0, nper, 2):
                axis.add_patch(
                    mpatches.Rectangle(
                        offs + np.array([pos + iseg * dxs, gap]),
                        dxs,
                        height,
                        color="red",
                        alpha=alpha,
                ...
                axis.add_patch(
                    mpatches.Rectangle(
                        offs + np.array([pos + iseg * dxs, -height - gap]),
                        dxs,
                        height,
                        color="blue",
                        alpha=alpha,
                ...
                axis.add_patch(
                    mpatches.Rectangle(
                        offs + np.array([pos + (iseg + 1) * dxs, gap]),
                        dxs,
                        height,
                        color="blue",
                        alpha=alpha,
                ...
                axis.add_patch(
                    mpatches.Rectangle(
                        offs + np.array([pos + (iseg + 1) * dxs, -height - gap]),
                        dxs,
                        height,
                        color="red",
                        alpha=alpha,
        ...
        if e.__class__ in [Monitor]:
            ...
            axis.add_patch(
                mpatches.Circle(
                    offs + offs_mon + np.array([pos, 0.0]),
                    h / 2,
                    color="black",
                    alpha=alpha * 0.5,
                )
    ...
    axis.add_patch(
        mpatches.Rectangle(
            offs - (0, lw / 2), 1.0, lw, color="black", alpha=alpha * 0.4
        )
    )
...
dict_plot = {
    Quadrupole: {"scale": 0.7, "color": "r", "edgecolor": "r", "label": "quad"},
    ...
    Drift: {"scale": 0.0, "color": "k", "edgecolor": "k", "label": ""},
    Marker: {"scale": 0.0, "color": "k", "edgecolor": "k", "label": "mark"},
...
def plot_elems(
    ...
    for elem in lat.sequence:
        ...
        annotation = ax.annotate(
            elem.__class__.__name__ + ": " + elem.id,
            xy=(L + l / 2.0 + s_point, 0),  # xycoords='data',
...
def plot_opt_func_reduced(
    lat, tws, top_plot=["Dx"], legend=True, fig_name=None, grid=False, font_size=18
):
    """
    function for plotting: lattice (bottom section), vertical and horizontal beta-functions (middle section),
    other parameters (top section) such as "Dx", "Dy", "E", "mux", "muy", "alpha_x", "alpha_y", "gamma_x", "gamma_y"
...
def compare_betas(
    ...
    leg = ax_top.legend(
        shadow=False, fancybox=True, prop=font_manager.FontProperties(size=font_size)
    )
    leg.get_frame().set_alpha(0.2)
...
def show_mu(contour_da, mux, muy, x_array, y_array, zones=None):
    ...
    fig1 = plt.contour(
        contour_da, 1, extent=extent, linewidths=2, colors="k"
    )  # , colors = 'r')
    fig1 = plt.contourf(mux, 40, cmap=my_cmap, extent=extent)  # , colors = 'r')
    cb = plt.colorbar()
    fig1 = plt.contourf(mux, 10, levels=[-1, -0.0001], colors="w", extent=extent)
    ...
    cb = plt.colorbar()
    fig1 = plt.contourf(muy, 10, levels=[-1, -0.0001], colors="w", extent=extent)
    plt.xlabel("X, m")
    plt.ylabel("Y, m")
    plt.grid(True)
    cb.set_label("Qy")
...
def show_e_beam(
    ...
    **kwargs,
    ...
    slice_params = global_slice_analysis(
        p_array_copy, nparts_in_slice, smooth_param, filter_base, filter_iter
    )
    ...
    plt.plot(
        slice_params.s * tau_factor,
        slice_params.exn * 1e6,
        "r",
        label=rf"$\varepsilon_x^{{\mathrm{{proj}}}} = {emitxn_mm_mrad}\,\mathrm{{mm\cdot{{}}mrad}}$",
    )
    plt.plot(
        slice_params.s * tau_factor,
        slice_params.eyn * 1e6,
        "b",
    ...
    leg = ax_c.legend(
        [imax_label], handlelength=0, handletextpad=0, fancybox=True, loc="best"
    )
    ...
    if show_moments:
        ...
        plt.plot(
            slice_params.s * tau_factor,
            (slice_params.my + slice_params.sig_y) * 1e3,
            "w",
            lw=1,
        )
        plt.plot(
    ...
    if headtail:
        ...
        anchored_text = AnchoredText(
            label, loc=location, prop=dict(fontsize=leg.get_texts()[0].get_fontsize())
        )
...
def show_phase_space(
    ...
    if inverse_tau:
        p_array_copy.tau()[:] *= -1
    slice_params = global_slice_analysis(
        p_array_copy, nparts_in_slice, smooth_param, 2, 2
    )
    ...
    if show_moments:
        ...
        plt.plot(
            slice_params.s * 1e3,
            (slice_params.myp + slice_params.sig_yp) * 1e3,
            "w",
            lw=1,
        )
        plt.plot(
    ...
    if show_moments:
        ...
        plt.plot(
            slice_params.s * 1e3,
            (slice_params.mxp + slice_params.sig_xp) * 1e3,
            "w",
            lw=1,
        )
        plt.plot(
...
def compare_beams(
    p_array_1,
    p_array_2,
    nparts_in_slice1=5000,
    nparts_in_slice2=5000,
    smoth_param=0.05,
    inverse_tau=False,
    nfig=40,
    title=None,
    ...
    slice_params1 = global_slice_analysis(
        p_array_copy1, nparts_in_slice1, smoth_param, 2, 2
    )
    slice_params2 = global_slice_analysis(
        p_array_copy2, nparts_in_slice2, smoth_param, 2, 2
    )
    ...
    plt.plot(
        slice_params1.s * 1e6,
        slice_params1.se * 1e-3,
        "r",
        label=rf"$\sigma_E$: {legend_beam1}",
    )
    plt.plot(
        slice_params2.s * 1e6,
        slice_params2.se * 1e-3,
        "b",
    ...
    plt.plot(
        ...
        slice_params1.I,
        "r",
        label=rf"$I_{{\mathrm{{max}}}}={i1max:.0f}\,\mathrm{{A}}$ {legend_beam1}",
    )
    plt.plot(
        slice_params2.s * 1e6,
        slice_params2.I,
        "b",
        label=rf"$I_{{\mathrm{{max}}}}={i2max:.0f}\,\mathrm{{A}}$ {legend_beam2}",
    )
    plt.legend()
    plt.setp(ax_c.get_xticklabels(), visible=False)
    plt.ylabel("$I\,[\mathrm{A}]$")
    ...
    plt.plot(
        slice_params1.s * 1e6,
        slice_params1.mp * 1e2,
        "r",
        label=r"$\Delta{} E/E$: " + legend_beam1,
    )
    plt.plot(
        slice_params2.s * 1e6,
        slice_params2.mp * 1e2,
        "b",
        label=r"$\Delta{} E/E$: " + legend_beam2,
    )
    plt.legend()
    plt.xlabel(tau_label)
    ...
    plt.plot(
        slice_params1.s * 1e6,
        slice_params1.mx * 1e6,
        "r",
        label=r"$x_\mathrm{slice}$: " + legend_beam1,
    )
    plt.plot(
        slice_params2.s * 1e6,
        slice_params2.mx * 1e6,
        "b",
        label=r"$x_\mathrm{slice}$: " + legend_beam2,
    )
    plt.plot(
        slice_params1.s * 1e6,
        slice_params1.my * 1e6,
        "r--",
        label=r"$y_\mathrm{slice}$: " + legend_beam1,
    )
    plt.plot(
        slice_params2.s * 1e6,
        slice_params2.my * 1e6,
        "b--",
        label=r"$y_\mathrm{slice}$: " + legend_beam2,
    )
    plt.legend()
...
def compare_beams_reduced(
    ...
    slice_params1 = global_slice_analysis(
        p_array_copy1, nparts_in_slice, smoth_param, 2, 2
    )
    slice_params2 = global_slice_analysis(
        p_array_copy2, nparts_in_slice, smoth_param, 2, 2
    )
...
def show_e_beam_slices(
    ...
    labelx = (
        r"$\varepsilon_x^{0} = $"
        + str(np.round(slice_params.ex[ind0], 2))
        + r" $\mu m \cdot rad$"
    )
    labely = (
        r"$\varepsilon_y^{0} = $"
        + str(np.round(slice_params.ey[ind0], 2))
        + r" $\mu m \cdot rad$"
    )
    if slice is None:
        fmx = "r"
    ...
    if headtail:
        leg = ax_c.legend(
            [label_arr], handlelength=0, handletextpad=0, fancybox=True, loc=location
        )
        for item in leg.legendHandles:
            item.set_visible(False)
    plt.setp(ax_c.get_xticklabels(), visible=False)
...
def beam_jointplot(
    ...
    if show_head:
        leg = ax_top.legend(
            [label], handlelength=0, handletextpad=0, fancybox=True, loc=location
        )
        for item in leg.legendHandles:
            item.set_visible(False)
