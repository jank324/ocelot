...
class Ray(object):
    ...
    def w(self):
        ...
        return (2.0 * pi * self.c) / self.lamb

    @w.setter
    def w(self, value):
        print("setter of w called")
        self.lamb = (2.0 * pi * self.c) / self.value
        ...
        if nr * nk > 0:
            ...
            if s_int < s and s_int > intersection_tol:
                ...
                if o.__class__ == EllipticMirror:
                    ...
                    c_ = rz**2 / az**2 + ry**2 / ay**2 - 1.0
                    d_ = b_**2 - 4 * a_ * c_

                    s1 = (-b_ + np.sqrt(d_)) / (2.0 * a_)
                    s2 = (-b_ - np.sqrt(d_)) / (2.0 * a_)
                    ...
                    ang = arctan2(1.0 / az * r_loc[2], 1.0 / ay * (-r_loc[1] + ay))

                    # debug(r_loc[2], r_loc[1] - ay)
                    debug("ellipse angle=", ang)
                    debug("local coord:", az * sin(ang), -ay * cos(ang) + ay)

                    no = np.array([0, cos(ang), -ay / az * sin(ang)]) / np.sqrt(
                        ay**2 / az**2 * sin(ang) ** 2 + cos(ang) ** 2
                    )
...
def refl_matrix(no):
    ...
    M = np.matrix(
        [
            [-1.0 + 2.0 * x**2, 2.0 * x * y, 2.0 * x * z],
            [2.0 * y * x, -1.0 + y**2 * (1.0 + 1), y * z * (1.0 + 1)],
            [2.0 * z * x, 2.0 * z * y, -1.0 + 2.0 * z**2],
        ]
    )
    return M
...
def trace(ray, geo):
    ...
    while n_reflect < n_reflect_max:
        ...
        if obj.__class__ == Mirror:
            ...
            debug("reflecting off", obj.id)
            debug(
                np.dot(obj.no, ray.k[-1])
                / (np.linalg.norm(obj.no) * np.linalg.norm(ray.k[-1]))
            )
            phi = np.arccos(
                np.dot(obj.no, ray.k[-1])
                / (np.linalg.norm(obj.no) * np.linalg.norm(ray.k[-1]))
            ...
            k_new = np.array(
                [
                    ray.k[-1][0] - r_loc[0] * ray.k[-1][2] / obj.f,
                    ray.k[-1][1] - r_loc[1] * ray.k[-1][2] / obj.f,
                    ray.k[-1][2],
                ]
            )
            s_new = ray.s[-1]
