...
class OptElement:
    ...
    def __init__(self, r, roll_ang, pitch_ang, yaw_ang, size, id):
        ...
        no = np.matrix([[0.0, 1.0, 0.0]]).T

        z_ax = np.matrix([[0.0, 0.0, 1.0]]).T

        self.pitch_ang = pitch_ang
        M1 = np.matrix(
            [
                [1.0, 0.0, 0.0],
                [0.0, np.cos(pitch_ang), np.sin(pitch_ang)],
                [0.0, -np.sin(pitch_ang), np.cos(pitch_ang)],
            ]
        )
        # if pitch_ang
        ...
        M2 = np.matrix(
            [
                [np.cos(yaw_ang), 0.0, np.sin(yaw_ang)],
                [0.0, 1.0, 0.0],
                [-np.sin(yaw_ang), 0.0, np.cos(yaw_ang)],
            ]
        )
        # if pitch_ang
...
class Detector:
    def __init__(
        self, r=[0, 0, 0], size=[1.0, 1.0, 0], no=[0, 0, 1], nx=101, ny=101, id=""
    ):
        self.r = r
        self.size = size
        self.nx = nx
