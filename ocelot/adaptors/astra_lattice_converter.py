...
class AstraLatticeConverter:
    def __init__(self):
        ...
        self.astra_matrix["QUADRUPOLE"] = {
            "type": Quadrupole,
            "eid_prefix": "Q_",
            "position": "Q_POS",
            "params": {"Q_K": "k1", "Q_LENGTH": "l"},
            "default_vals": {"Q_BORE": 0.0000001},
        }
    ...
    def astra2ocelot(self, filename):
        ...
        for line in lines:
            ...
            if flag:
                ...
                # parse element name
                if "eid_prefix" in self.astra_matrix[element_type].keys() and (
                    self.astra_matrix[element_type]["eid_prefix"] != ""
                    or self.astra_matrix[element_type]["eid_prefix"] != None
                ):
                    element_name = (
                        self.astra_matrix[element_type]["eid_prefix"] + result[0][1]
                    )
                else:
                ...
                if element_name not in element_name_array:
                    ...
                    elements_list.append(
                        [
                            self.astra_matrix[element_type]["type"](eid=element_name),
                            None,
                        ]
                    )
                ...
                if result[0][0] in self.astra_matrix[element_type]["params"]:
                    ...
                    elements_list[element_index][0].__dict__[param] = float(
                        result[0][2]
                    )
        ...
        for i, elem in enumerate(lattice.sequence):
            ...
            if elem_class in self.astra_matrix:
                ...
                for j in self.astra_matrix[elem_class]["params"]:
                    tmp_data[j] = elem.__dict__[
                        self.astra_matrix[elem_class]["params"][j]
                    ]
