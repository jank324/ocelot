        ...
        if line.strip().startswith("#"):
            ...
            try:
                ...
                if type == "intensity":
                    ...
                    intens.intensity[round(w0 + iSpec * dw, 4)][i, j] = float(
                        line.strip()
                    )
...
def calculateSR_py(lat, beam, screen, runParameters):
    ...
    for e in lat.sequence:
        if e.type == "undulator":
            ...
            fields.append(
                SRWLMagFldU(
                    [
                        SRWLMagFldH(1, "v", By, phBy, sBy, 1),
                        SRWLMagFldH(1, "h", Bx, phBx, sBx, 1),
                    ],
                    undPer,
                    numPer,
                )
    ...
    magFldCnt = SRWLMagFldC(
        fields, array("d", arxc), array("d", aryc), array("d", arzc)
    ...
    zStartInteg = (
        zstart  # longitudinal position to start integration (effective if < zEndInteg)
    )
    zEndInteg = (
        zend  # longitudinal position to finish integration (effective if > zStartInteg)
    )
    ...
    elecBeam.partStatMom1.z = (
        zstart  # Initial Longitudinal Coordinate (set before the ID)
    )
    ...
    srwl.CalcIntFromElecField(
        arI1, wfr1, 6, 0, 6, wfr1.mesh.eStart, wfr1.mesh.xStart, wfr1.mesh.yStart
    )
    print("done")
