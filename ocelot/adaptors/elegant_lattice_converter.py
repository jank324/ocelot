...
class ElegantLatticeConverter:
    ...
    def init_convert_matrix(self):
        ...
        self.elegant_matrix["QUAD"] = {
            "type": Quadrupole,
            "params": {"L": "l", "K1": "k1", "K2": "k2", "TILT": "tilt"},
        }
        self.elegant_matrix["SEXT"] = {
            "type": Sextupole,
            "params": {"L": "l", "K2": "k2"},
        }
        ...
        self.elegant_matrix["WIGGLER"] = {
            "type": Undulator,
            "params": {"L": "l", "K": "Kx", "POLES": ["nperiods", "0.5"]},
        }
        self.elegant_matrix["EMATRIX"] = {
            "type": Matrix,
            "params": {
                "L": "l",
                "R11": ["r", 0, 0],
    ...
    def elegant2ocelot(self, file_name):
        ...
        for line in lines:
            ...
            if string[0] == "%" and string.find("STO") != -1:
                ...
                constants[expr[1].strip()] = self.calc_rpn(
                    expr[0].strip(), constants, "In string " + string
                )
                continue
        ...
        if used_cell_name in elegant_elements_dict:
            ...
            print(
                "********* ERROR! used unkown cell or element "
                + used_cell_name
                + " *********"
            )
            sys.exit()
        ...
        for elem in elegant_cell:
            if elem not in elements_list.keys():
                ...
                if param[0] in self.elegant_matrix.keys():
                    # create element
                    elements_list[elem] = self.elegant_matrix[param[0]]["type"](
                        eid=elem
                    )
                    ...
                    for data in param:
                        ...
                        if result[0] in self.elegant_matrix[param[0]]["params"]:
                            ...
                            if tmp.__class__ == list:
                                ...
                                # fix for phi cavity
                                if (
                                    elements_list[elem].__class__ == Cavity
                                    and tmp == "phi"
                                ):
                                    val = 90.0 - val
                                elements_list[elem].element.__dict__[tmp] = val
                    if elements_list[elem].__class__ == Undulator:
                        elements_list[elem].element.lperiod = (
                            elements_list[elem].element.l
                            / elements_list[elem].element.nperiods
                        )
        ...
        while check_lines:
            ...
            for i in range(0, len(elements_list)):
                ...
                if elements_list[i] not in elements_dict:
                    print(
                        "********* ERROR! used undefined cell or element "
                        + elements_list[i]
                        + " *********"
                    )
                    sys.exit()
    ...
    def ocelot2elegant(self, lattice, file_name="lattice.lte"):
        ...
        for elegant_type in self.elegant_matrix:
            reverse_matrix.update(
                {self.elegant_matrix[elegant_type]["type"].__name__: elegant_type}
            )
            elements_arr.update(
                {self.elegant_matrix[elegant_type]["type"].__name__: []}
            )
        ...
        for element_class in elements_arr:
            for elem in elements_arr[element_class]:
                ...
                if "L" in self.elegant_matrix[reverse_matrix[element_class]]["params"]:
                    tmp = self.elegant_matrix[reverse_matrix[element_class]]["params"][
                        "L"
                    ]
                    lines += ",L=" + str(elem.element.__dict__[tmp])

                # print other parameters
                for param in self.elegant_matrix[reverse_matrix[element_class]][
                    "params"
                ]:
                    if param == "L":
                        continue
                    ...
                    if tmp.__class__ == list:
                        if len(tmp) == 2:
                            lines += (
                                ...
                                + param
                                + "="
                                + str(elem.element.__dict__[tmp[0]] / float(tmp[1]))
                            )
                        elif len(tmp) == 3:
                            lines += (
                                ","
                                + param
                                + "="
                                + str(elem.element.__dict__[tmp[0]][tmp[1], tmp[2]])
                            )
                    else:
                        # fix for phi cavity
                        if elem.__class__ == Cavity and tmp == "phi":
                            ...
                            lines += (
                                ...
                                + param
                                + "="
                                + str(elem.element.__dict__["r"][i, j])
                            )
                        else:
